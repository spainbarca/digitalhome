...
class UbigeoDepartamentosSeeder extends Seeder
{
    public function run()
    {
        $path = database_path('seeders/data/ubigeo_departamento.csv');
        ...
        DB::table('ubigeo_departamentos')->delete();
        ...
        foreach ($rows as $row) {
            ...
            $data[] = [
                ...
                'altitude' => is_numeric(trim($item['altitude'])) ? trim($item['altitude']) : 0,
        ...
        foreach (array_chunk($data, 500) as $chunk) {
            DB::table('ubigeo_departamentos')->insert($chunk);
        }
    }
