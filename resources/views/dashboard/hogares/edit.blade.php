...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <form method="POST" action="{{ route('dashboard.hogares.update', $hogar) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="trezo-card-content">
                        <div class="grid grid-cols-1 gap-[20px]">
                            ...
                            <div>
                                ...
                                <input type="text" name="nombre" value="{{ old('nombre', $hogar->nombre) }}" placeholder="Ej: Casa Familia García" maxlength="100"
                            ...
                            <div>
                                ...
                                @if ($hogar->avatar_url)
                                    <div class="mb-[10px] flex items-center gap-[12px]">
                                        <img src="{{ $hogar->avatar_url }}" alt="{{ $hogar->nombre }}"
                            ...
                            <div>
                                ...
                                <textarea name="descripcion" rows="3" placeholder="Descripción opcional del hogar..."
                                    class="rounded-md text-black dark:text-white border @error('descripcion') border-danger-500 @else border-gray-200 dark:border-[#172036] @enderror bg-white dark:bg-[#0c1427] px-[17px] py-[14px] block w-full outline-0 placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:border-primary-500">{{ old('descripcion', $hogar->descripcion) }}</textarea>
                                @error('descripcion')
                                    <p class="mt-[6px] text-danger-500 text-sm">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                <div class="flex items-center gap-[15px]">
                    ...
                    <a href="{{ route('dashboard.hogares.show', $hogar) }}" class="font-medium inline-block transition-all rounded-md py-[10px] px-[20px] border border-gray-200 dark:border-[#172036] text-black dark:text-white hover:bg-gray-50 dark:hover:bg-[#15203c]">
                        Cancelar
                    </a>
                </div>
            </form>
