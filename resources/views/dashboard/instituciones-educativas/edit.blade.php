    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Editar Institución Educativa</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.instituciones-educativas.show', $institucion) }}" class="transition-all hover:text-primary-500 truncate">{{ $nombreEfectivo }}</a>
                    </li>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                ...
                <div class="trezo-card-content">
                    <form method="POST" action="{{ route('dashboard.instituciones-educativas.update', $institucion) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-[25px]">
                            ...
                            <div class="grid grid-cols-12 gap-[20px] md:gap-[25px]">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="hidden" name="empresa_id" id="empresa_id" value="{{ old('empresa_id', $institucion->empresa_id ?? '') }}">
                        ...
                        {{-- ── SECCIÓN 2: Tipo de Institución ─────────────────────── --}}
                        <div class="mb-[25px]">
                            <div class="flex items-center gap-[8px] mb-[15px] pb-[12px] border-b border-gray-100 dark:border-[#172036]">
                                <i class="material-symbols-outlined !text-[20px] text-primary-500">school</i>
                                <h6 class="!mb-0 font-semibold text-black dark:text-white">2. Tipo de Institución</h6>
                                <span class="text-xs text-gray-400">Opcional</span>
                            </div>
                            <div class="max-w-lg">
                                <label class="mb-[10px] text-black dark:text-white font-medium block">Tipo de Institución</label>
                                <div class="relative" id="tipoInstWrapper">
                                    <div id="tipoInstTrigger"
                                        class="h-[55px] flex items-center rounded-md border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[14px] cursor-pointer select-none transition-all hover:border-primary-500 {{ $errors->has('tipo_institucion_educativa_id') ? 'border-danger-500' : '' }}">
                                        <i class="material-symbols-outlined !text-[20px] mr-[8px] text-gray-400" id="tipoInstIconPreview">school</i>
                                        <span id="tipoInstLabel" class="text-gray-500 dark:text-gray-400 text-sm flex-1 truncate">Sin tipo asignado</span>
                                        <i class="material-symbols-outlined !text-[20px] text-gray-400 transition-transform duration-200" id="tipoInstChevron">expand_more</i>
                                    </div>
                                    <input type="hidden" name="tipo_institucion_educativa_id" id="tipo_institucion_educativa_id"
                                        value="{{ old('tipo_institucion_educativa_id', $institucion->tipo_institucion_educativa_id) }}">
                                    <div id="tipoInstDropdown"
                                        class="hidden absolute z-[50] w-full bg-white dark:bg-[#0c1427] border border-gray-200 dark:border-[#172036] rounded-md shadow-lg mt-[4px]">
                                        <div class="p-[8px] border-b border-gray-100 dark:border-[#172036]">
                                            <input type="text" id="tipoInstBuscar" placeholder="Buscar tipo..."
                                                class="w-full px-[10px] py-[6px] text-sm border border-gray-200 dark:border-[#172036] rounded-md bg-white dark:bg-[#0c1427] text-black dark:text-white outline-0 focus:border-primary-500 placeholder:text-gray-400">
                                        </div>
                                        <ul id="tipoInstOpciones" class="max-h-[240px] overflow-y-auto py-[4px]">
                                            <li class="tipo-inst-opcion flex items-center gap-[8px] px-[12px] py-[9px] cursor-pointer hover:bg-primary-50 dark:hover:bg-[#15203c] transition-colors"
                                                data-id="" data-nombre="Sin tipo asignado" data-icono="school">
                                                <i class="material-symbols-outlined !text-[18px] text-gray-400">school</i>
                                            ...
                                            <li class="tipo-inst-opcion flex items-center gap-[8px] px-[12px] py-[9px] cursor-pointer hover:bg-primary-50 dark:hover:bg-[#15203c] transition-colors"
                                                ...
                                                data-icono="{{ $tipo->icono ?? 'school' }}">
                                                <i class="material-symbols-outlined !text-[18px] text-primary-500">{{ $tipo->icono ?? 'school' }}</i>
                                ...
                                @error('tipo_institucion_educativa_id')<p class="text-danger-500 text-xs mt-[5px]">{{ $message }}</p>@enderror
                            </div>
                        </div>

                        {{-- ── SECCIÓN 3: Datos de la Institución ──────────────────── --}}
                        <div class="mb-[25px]">
                            <div class="flex items-center gap-[8px] mb-[15px] pb-[12px] border-b border-gray-100 dark:border-[#172036]">
                                <i class="material-symbols-outlined !text-[20px] text-primary-500">info</i>
                                <h6 class="!mb-0 font-semibold text-black dark:text-white">3. Datos de la Institución</h6>
                            </div>
                            <div class="grid grid-cols-1 gap-[20px] md:gap-[25px]">
                                ...
                                <div class="max-w-lg">
                                    <label class="mb-[10px] text-black dark:text-white font-medium block">
                                        Nombre Referencial
                                        <span class="text-xs text-gray-400 font-normal ml-[4px]">(alias para identificarla en el hogar)</span>
                                    </label>
                                    <input type="text" name="nombre_referencial" value="{{ old('nombre_referencial', $institucion->nombre_referencial) }}" maxlength="255"
                                        placeholder="Ej: Colegio de los niños"
                                ...
                                <div class="max-w-lg">
                                    ...
                                    <textarea name="notas" rows="3" maxlength="5000"
                                        placeholder="Información adicional de la institución..."
                                        class="rounded-md text-black dark:text-white border {{ $errors->has('notas') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] py-[14px] block w-full outline-0 transition-all placeholder:text-gray-500 focus:border-primary-500 resize-none">{{ old('notas', $institucion->notas) }}</textarea>
                        ...
                        <div class="mb-[25px]">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[20px] md:gap-[25px]">
                                ...
                                <div>
                                    <label class="mb-[10px] text-black dark:text-white font-medium block">
                                        Logo <span class="text-xs text-gray-400 font-normal">(PNG/JPG, máx. 10 MB)</span>
                                    </label>
                                    @if($institucion->logo_path)
                                    <div class="mb-[10px] flex items-center gap-[10px]">
                                        <img src="{{ Storage::url($institucion->logo_path) }}" id="logoPreview"
                                            class="w-[64px] h-[64px] rounded-md object-contain border border-gray-100 dark:border-[#172036]" alt="Logo actual">
                                    ...
                                    <div id="logoPreviewWrap" class="hidden mb-[10px]">
                                        <img id="logoPreview" src="" alt="Preview logo" class="w-[64px] h-[64px] rounded-md object-contain border border-gray-100 dark:border-[#172036]">
                                    </div>
                                    @endif
                                ...
                                <div>
                                    <label class="mb-[10px] text-black dark:text-white font-medium block">
                                        Imagen de fondo <span class="text-xs text-gray-400 font-normal">(PNG/JPG, máx. 10 MB)</span>
                                    </label>
                                    @if($institucion->imagen_path)
                                    <div class="mb-[10px] flex items-center gap-[10px]">
                                        <img src="{{ Storage::url($institucion->imagen_path) }}" id="imagenPreview"
                                            class="w-[80px] h-[50px] rounded-md object-cover" alt="Imagen actual">
                                    ...
                                    <div id="imagenPreviewWrap" class="hidden mb-[10px]">
                                        <img id="imagenPreview" src="" alt="Preview imagen" class="w-[80px] h-[50px] rounded-md object-cover">
                                    </div>
                                    @endif
                                ...
                                <div class="flex items-center gap-[12px] pt-[10px] lg:pt-[30px]">
                                    ...
                                    <label class="relative cursor-pointer">
                                        <input type="checkbox" name="activo" value="1" class="sr-only peer" {{ old('activo', $institucion->activo) ? 'checked' : '' }}>
                        ...
                        <div class="mt-[20px] flex justify-end gap-[15px]">
                            <a href="{{ route('dashboard.instituciones-educativas.show', $institucion) }}"
        ...
        <script>
            ...
            (function () {
                const wrapper  = document.getElementById('tipoInstWrapper');
                const trigger  = document.getElementById('tipoInstTrigger');
                const dropdown = document.getElementById('tipoInstDropdown');
                const chevron  = document.getElementById('tipoInstChevron');
                const buscar   = document.getElementById('tipoInstBuscar');
                const hidden   = document.getElementById('tipo_institucion_educativa_id');
                const label    = document.getElementById('tipoInstLabel');
                const iconPrev = document.getElementById('tipoInstIconPreview');
                const opciones = document.querySelectorAll('.tipo-inst-opcion');
