...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Editar Cuenta — {{ $cuenta->numero_cuenta }}</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Editar Cuenta de Servicio</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.cuentas-servicio.show', $cuenta) }}" class="transition-all hover:text-primary-500">{{ $cuenta->numero_cuenta }}</a>
                    </li>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="trezo-card-header bg-gray-50 dark:bg-[#15203c] mb-[20px] md:mb-[25px] flex items-center justify-between -mx-[20px] md:-mx-[25px] -mt-[20px] md:-mt-[25px] p-[20px] md:p-[25px] rounded-t-md">
                    <h5 class="!mb-0">Editar: {{ $cuenta->numero_cuenta }}</h5>
                </div>

                <div class="trezo-card-content">
                    <form method="POST" action="{{ route('dashboard.cuentas-servicio.update', $cuenta) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                            ...
                            <div>
                                <label class="mb-[10px] text-black dark:text-white font-medium block">Tipo de Servicio</label>
                                <div class="relative" id="tipoServicioWrapper">
                                    ...
                                    <input type="hidden" name="tipo_servicio_id" id="tipo_servicio_id"
                                        value="{{ old('tipo_servicio_id', $cuenta->proveedor?->tipo_servicio_id ?? '') }}">
                                    <div id="tipoServicioDropdown" class="hidden absolute z-[50] w-full bg-white dark:bg-[#0c1427] border border-gray-200 dark:border-[#172036] rounded-md shadow-lg mt-[4px]">
                                        ...
                                        <ul id="tipoServicioOpciones" class="max-h-[210px] overflow-y-auto py-[4px]">
                                            ...
                                            <li class="tipo-opcion flex items-center gap-[8px] px-[12px] py-[9px] cursor-pointer hover:bg-primary-50 dark:hover:bg-[#15203c] transition-colors"
                                                data-id="{{ $tipo->id }}" data-nombre="{{ $tipo->nombre }}" data-icono="{{ $tipo->icono ?? 'category' }}">
                            ...
                            <div>
                                ...
                                <select name="proveedor_id" id="proveedor_id"
                                    ...
                                    @foreach($proveedores as $proveedor)
                                        <option value="{{ $proveedor->id }}" data-tipo="{{ $proveedor->tipo_servicio_id }}"
                                            {{ old('proveedor_id', $cuenta->proveedor_id) == $proveedor->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="numero_cuenta" value="{{ old('numero_cuenta', $cuenta->numero_cuenta) }}"
                            ...
                            <!-- Titular -->
                            <div>
                                <label class="mb-[10px] text-black dark:text-white font-medium block">Titular del Contrato</label>
                                <select name="titular_user_id"
                                    ...
                                    @foreach($hogarUsers as $u)
                                        <option value="{{ $u->id }}" {{ old('titular_user_id', $cuenta->titular_user_id) == $u->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="estado"
                                    ...
                                    <option value="activa"     {{ old('estado', $cuenta->estado) === 'activa'     ? 'selected' : '' }}>Activa</option>
                                    <option value="suspendida" {{ old('estado', $cuenta->estado) === 'suspendida' ? 'selected' : '' }}>Suspendida</option>
                                    <option value="cancelada"  {{ old('estado', $cuenta->estado) === 'cancelada'  ? 'selected' : '' }}>Cancelada</option>
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="fecha_inicio"
                                    value="{{ old('fecha_inicio', $cuenta->fecha_inicio?->format('Y-m-d')) }}"
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="notas" rows="3" placeholder="Información adicional..."
                                    class="rounded-md text-black dark:text-white border {{ $errors->has('notas') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] py-[12px] block w-full outline-0 transition-all placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:border-primary-500 resize-none">{{ old('notas', $cuenta->notas) }}</textarea>
        ...
        <script>
            (function () {
                ...
                trigger.addEventListener('click', () => dropdown.classList.contains('hidden') ? abrirDropdown() : cerrarDropdown());
                buscar.addEventListener('input', () => {
                    const q = buscar.value.toLowerCase();
                    opciones.forEach(li => { li.style.display = li.dataset.nombre.toLowerCase().includes(q) ? '' : 'none'; });
                });
                opciones.forEach(li => {
                    li.addEventListener('click', () => { aplicarSeleccion(li.dataset.id, li.dataset.nombre, li.dataset.icono); cerrarDropdown(); filtrarProveedores(); });
                });
            ...
            const currentProveedorId = "{{ $cuenta->proveedor_id }}";
            ...
            function filtrarProveedores() {
                ...
                todosProveedores
                    ...
                    .forEach(p => {
                        ...
                        if (p.id === prevVal || (!prevVal && p.id === currentProveedorId)) opt.selected = true;
