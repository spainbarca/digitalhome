...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Nueva Cuenta de Servicio</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                ...
                <div class="trezo-card-content">
                    <form method="POST" action="{{ route('dashboard.cuentas-servicio.store') }}">
                        @csrf
                        ...
                        <h6 class="font-semibold text-black dark:text-white mb-[16px] pb-[10px] border-b border-gray-100 dark:border-[#172036] flex items-center gap-[8px]">
                            <i class="material-symbols-outlined !text-[18px] text-primary-500">home</i>
                            Propiedad
                        </h6>

                        <div class="mb-[25px]">
                            <label class="mb-[10px] text-black dark:text-white font-medium block">
                                Propiedad <span class="text-danger-500">*</span>
                            </label>
                            <select name="propiedad_id" id="propiedad_id"
                                class="h-[55px] rounded-md text-black dark:text-white border {{ $errors->has('propiedad_id') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[14px] block w-full outline-0 cursor-pointer transition-all focus:border-primary-500">
                                <option value="">Selecciona una propiedad...</option>
                                @foreach($propiedades as $prop)
                                    <option value="{{ $prop->id }}"
                                        {{ old('propiedad_id', $propiedadSeleccionada?->id) == $prop->id ? 'selected' : '' }}>
                                        {{ $prop->alias }}
                                        @if($prop->direccion) — {{ Str::limit($prop->direccion, 50) }} @endif
                                    </option>
                                @endforeach
                            </select>
                            @error('propiedad_id')<p class="text-danger-500 text-xs mt-[5px]">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div class="mt-[25px] flex justify-end gap-[15px]">
                            <a href="{{ route('dashboard.cuentas-servicio.index', $propiedadSeleccionada ? ['propiedad' => $propiedadSeleccionada->id] : []) }}"
        ...
        <script>
            ...
            if (document.getElementById('tipo_servicio_id').value) filtrarProveedores();
        </script>
    </body>
</html>
