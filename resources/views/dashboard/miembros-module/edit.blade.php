        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                ...
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.miembros.index', ['hogar' => $miembro->hogar_id]) }}"
            ...
            <form method="POST" action="{{ route('dashboard.miembros.update', $miembro) }}">
                @csrf
                @method('PUT')
                ...
                <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                    <div class="trezo-card-header bg-gray-50 dark:bg-[#15203c] mb-[20px] md:mb-[25px] -mx-[20px] md:-mx-[25px] -mt-[20px] md:-mt-[25px] p-[20px] md:p-[25px] rounded-t-md flex items-center gap-[14px]">
                        <!-- Mini avatar -->
                        <div class="w-[44px] h-[44px] rounded-full border-[2px] border-white dark:border-[#172036] bg-primary-100 dark:bg-[#15203c] overflow-hidden flex items-center justify-center flex-shrink-0">
                            @if($persona?->avatar_url)
                                <img src="{{ $persona->avatar_url }}" class="w-full h-full object-cover rounded-full" alt="">
                            @else
                                <span class="text-primary-500 font-bold">{{ strtoupper(substr($persona?->nombres ?? '?', 0, 1)) }}</span>
                            @endif
                        </div>
                        <div>
                            <h5 class="!mb-0">
                            ...
                            <p class="!mb-0 text-sm text-gray-500 dark:text-gray-400 mt-[2px]">
                                Hogar: {{ $miembro->hogar?->nombre ?? '–' }}
                            </p>
                        </div>
                    </div>
                    <div class="trezo-card-content">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
                            ...
                            <div>
                                <label class="block mb-[10px] text-black dark:text-white font-medium">Persona</label>
                                <div class="h-[55px] rounded-md border border-gray-200 dark:border-[#172036] bg-gray-50 dark:bg-[#15203c] px-[17px] flex items-center text-gray-500 dark:text-gray-400">
                                    {{ $persona ? trim($persona->nombres . ' ' . $persona->apellido_paterno . ' ' . $persona->apellido_materno) : '–' }}
                                </div>
                            </div>
                                ...
                                <select name="parentesco_id"
                                    ...
                                    @foreach($parentescos as $parentesco)
                                        <option value="{{ $parentesco->id }}"
                                            {{ old('parentesco_id', $miembro->parentesco_id) == $parentesco->id ? 'selected' : '' }}>
                                            {{ $parentesco->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="apodo" value="{{ old('apodo', $miembro->apodo) }}"
                                    ...
                                    class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 transition-all focus:border-primary-500 placeholder:text-gray-500 dark:placeholder:text-gray-400">
                            </div>
