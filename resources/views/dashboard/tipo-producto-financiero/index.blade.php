...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Tipos de Producto Financiero</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Tipos de Producto Financiero</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">Tipos de Producto Financiero</li>
                </ol>
            </div>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-gray-100 dark:border-[#172036]">
                                    ...
                                    <th class="text-left py-[13px] px-[16px] text-sm font-semibold text-gray-500 dark:text-gray-400 w-[120px]">Naturaleza</th>
                            ...
                            <tbody id="tablaBody">
                                @forelse($tipos as $t)
                                @php
                                    $natLabel = ['activo' => 'Activo', 'pasivo' => 'Pasivo'];
                                    $natClass = [
                                        'activo' => 'text-blue-700 border-blue-500 bg-blue-100',
                                        'pasivo' => 'text-red-700 border-red-500 bg-red-100',
                                    ];
                                @endphp
                                <tr class="border-b border-gray-100 dark:border-[#172036] hover:bg-gray-50 dark:hover:bg-[#15203c] transition-colors"
                                    ...
                                    <td class="py-[13px] px-[16px]">
                                        <i class="material-symbols-outlined !text-[22px] text-primary-500">{{ $t->icono ?? 'savings' }}</i>
                                    </td>
                                ...
                                <tr id="filaVacia">
                                    <td colspan="6" class="text-center py-[50px] text-gray-500 dark:text-gray-400">
                                        <i class="material-symbols-outlined !text-[48px] text-gray-300 dark:text-gray-600 block mb-[10px]">savings</i>
                                        No hay tipos de producto financiero registrados.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
        ...
        <div id="modalTipo" class="hidden fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="relative bg-white dark:bg-[#0c1427] rounded-md w-full max-w-md mx-[16px] shadow-2xl z-10 max-h-[90vh] overflow-y-auto">
                ...
                <div class="flex items-center justify-between px-[25px] py-[18px] border-b border-gray-100 dark:border-[#172036] sticky top-0 bg-white dark:bg-[#0c1427] z-10">
                    <h5 class="!mb-0 text-[16px]" id="modalTitulo">Nuevo Tipo de Producto Financiero</h5>
                ...
                <div class="px-[25px] py-[20px] space-y-[16px]">
                    ...
                    <div>
                        ...
                        <input type="text" id="modalNombre" maxlength="255" placeholder="Ej: Cuenta de Ahorros"
                    ...
                    <div>
                        <label class="mb-[8px] text-black dark:text-white font-medium block text-sm">Naturaleza</label>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-[10px]">
                            <input type="text" id="modalIcono" maxlength="100" placeholder="Ej: savings"
                            ...
                            <div class="flex-shrink-0 w-[46px] h-[46px] rounded-md border border-gray-200 dark:border-[#172036] flex items-center justify-center bg-primary-50 dark:bg-[#15203c]">
                                <i class="material-symbols-outlined !text-[22px] text-primary-500" id="previewIcono">savings</i>
                            </div>
                        </div>
        ...
        <script>
            ...
            const storeUrl   = '{{ route('dashboard.tipo-producto-financiero.store') }}';
            const baseUrl    = '{{ url('dashboard/tipo-producto-financiero') }}';
            const iconDefault = 'savings';
            let   modoEdicion = false;
            let   idEditando  = null;

            const natLabels  = { activo: 'Activo', pasivo: 'Pasivo' };
            const natClasses = {
                activo: 'text-blue-700 border-blue-500 bg-blue-100',
                pasivo: 'text-red-700 border-red-500 bg-red-100',
            };
            ...
            function abrirModalCrear() {
                ...
                document.getElementById('modalTitulo').textContent  = 'Nuevo Tipo de Producto Financiero';
                document.getElementById('modalId').value            = '';
                document.getElementById('modalNombre').value        = '';
                document.getElementById('modalDescripcion').value   = '';
                document.getElementById('modalNaturaleza').value    = '';
            ...
            function abrirModalEditar(fila) {
                ...
                document.getElementById('modalTitulo').textContent  = 'Editar Tipo de Producto Financiero';
                ...
                document.getElementById('modalNaturaleza').value    = fila.dataset.naturaleza;
            ...
            function actualizarFila(t) {
                ...
                tds[3].textContent = t.descripcion || '—';
            ...
            function mostrarFilaVacia() {
                ...
                tr.innerHTML = `<td colspan="6" class="text-center py-[50px] text-gray-500">
                    <i class="material-symbols-outlined !text-[48px] text-gray-300 block mb-[10px]">savings</i>
                    No hay tipos de producto financiero registrados.</td>`;
                document.getElementById('tablaBody').appendChild(tr);
            }

            function mostrarErrores(errors) {
                const mapa = { nombre: 'errNombre', descripcion: 'errDescripcion', naturaleza: 'errNaturaleza', icono: 'errIcono' };
            ...
            function limpiarErrores() {
                ['errNombre', 'errDescripcion', 'errNaturaleza', 'errIcono'].forEach(id => {
                    const el = document.getElementById(id);
                    el.textContent = '';
