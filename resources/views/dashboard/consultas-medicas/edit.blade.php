        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                ...
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.consultas-medicas.show', $consulta) }}" class="transition-all hover:text-primary-500">{{ $consulta->fecha?->format('d/m/Y') }}</a>
                    </li>
            ...
            <form method="POST" action="{{ route('dashboard.consultas-medicas.update', $consulta) }}">
                @csrf @method('PUT')
                ...
                <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-[20px] md:gap-[25px]">
                        <div class="sm:col-span-1">
                            ...
                            <select name="hogar_miembro_id" id="selMiembro" class="w-full">
                                ...
                                @foreach($miembros as $m)
                                    ...
                                    <option value="{{ $m->id }}" {{ old('hogar_miembro_id', $consulta->hogar_miembro_id) == $m->id ? 'selected' : '' }}>{{ trim($lbl) }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="fecha" value="{{ old('fecha', $consulta->fecha?->format('Y-m-d')) }}"
                        ...
                        <div>
                            ...
                            <input type="time" name="hora" value="{{ old('hora', $consulta->hora ? substr($consulta->hora, 0, 5) : '') }}"
                        ...
                        <div>
                            ...
                            <select name="medico_id" id="selMedico" class="w-full">
                                ...
                                @foreach($medicos as $med)
                                    <option value="{{ $med->id }}" {{ old('medico_id', $consulta->medico_id) == $med->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="centro_medico_id" id="selCentro" class="w-full">
                                ...
                                @foreach($centrosMedicos as $cm)
                                    @php $nc = $cm->nombre_referencial ?? $cm->empresa?->razon_social ?? '(Sin nombre)'; @endphp
                                    <option value="{{ $cm->id }}" {{ old('centro_medico_id', $consulta->centro_medico_id) == $cm->id ? 'selected' : '' }}>{{ $nc }}</option>
                                @endforeach
                            </select>
                            ...
                            <select name="especialidad_medica_id" id="selEspecialidad" class="w-full">
                                ...
                                @foreach($especialidades as $esp)
                                    <option value="{{ $esp->id }}" data-icono="{{ $esp->icono ?? 'stethoscope' }}"
                                        {{ old('especialidad_medica_id', $consulta->especialidad_medica_id) == $esp->id ? 'selected' : '' }}>
                                        {{ $esp->nombre }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                    ...
                    <div class="grid grid-cols-1 gap-[20px] md:gap-[25px]">
                        <div>
                            ...
                            <input type="text" name="motivo" value="{{ old('motivo', $consulta->motivo) }}" maxlength="255"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                            <div>
                                ...
                                <textarea name="diagnostico" rows="4"
                                    class="rounded-md text-black dark:text-white border {{ $errors->has('diagnostico') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] py-[14px] block w-full outline-0 placeholder:text-gray-400 focus:border-primary-500 resize-none">{{ old('diagnostico', $consulta->diagnostico) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="tratamiento_indicado" rows="4"
                                    class="rounded-md text-black dark:text-white border {{ $errors->has('tratamiento_indicado') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] py-[14px] block w-full outline-0 placeholder:text-gray-400 focus:border-primary-500 resize-none">{{ old('tratamiento_indicado', $consulta->tratamiento_indicado) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="notas" rows="2"
                                class="rounded-md text-black dark:text-white border {{ $errors->has('notas') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] py-[14px] block w-full outline-0 placeholder:text-gray-400 focus:border-primary-500 resize-none">{{ old('notas', $consulta->notas) }}</textarea>
                ...
                <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px] max-w-lg">
                        <div>
                            ...
                            <select name="moneda_id" id="selMoneda" class="w-full">
                                ...
                                @foreach($monedas as $mon)
                                    <option value="{{ $mon->id }}" {{ old('moneda_id', $consulta->moneda_id) == $mon->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="costo" value="{{ old('costo', $consulta->costo) }}" min="0" step="0.01"
                                class="h-[55px] rounded-md text-black dark:text-white border {{ $errors->has('costo') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                ...
                <div class="trezo-card mb-[25px] bg-white dark:bg-[#0c1427] p-[20px] md:p-[25px] rounded-md">
                    <div class="flex justify-end gap-[15px]">
                        <a href="{{ route('dashboard.consultas-medicas.show', $consulta) }}"
