    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Consultas Médicas</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">Consultas</li>
                </ol>
            </div>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <form method="GET" action="{{ route('dashboard.consultas-medicas.index') }}" class="flex flex-wrap items-end gap-[12px]">
                    <div class="flex-1 min-w-[200px]">
                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-[6px]">Buscar</label>
                        <div class="relative">
                            <label class="leading-none absolute ltr:left-[13px] text-gray-400 mt-px top-1/2 -translate-y-1/2">
                                <i class="material-symbols-outlined !text-[18px]">search</i>
                            </label>
                            <input type="text" name="search" value="{{ $search }}" placeholder="Motivo o diagnóstico..."
                                class="bg-gray-50 dark:bg-[#15203c] border border-gray-50 dark:border-[#15203c] h-[36px] text-xs rounded-md w-full block text-black dark:text-white pl-[38px] pr-[13px] placeholder:text-gray-400 outline-0">
                        </div>
                    </div>
                    <div class="min-w-[160px]">
                        ...
                        <select name="hogar_miembro_id"
                            ...
                            @foreach($miembros as $m)
                                ...
                                <option value="{{ $m->id }}" {{ $miembroId == $m->id ? 'selected' : '' }}>{{ trim($label) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="min-w-[160px]">
                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-[6px]">Especialidad</label>
                        <select name="especialidad_medica_id"
                            class="h-[36px] text-xs rounded-md border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#15203c] text-black dark:text-white px-[10px] outline-0 cursor-pointer w-full">
                            <option value="">Todas</option>
                            @foreach($especialidades as $e)
                                <option value="{{ $e->id }}" {{ $especialidadId == $e->id ? 'selected' : '' }}>{{ $e->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div class="flex gap-[8px]">
                        ...
                        @if($search || $miembroId || $especialidadId || $fechaDesde || $fechaHasta)
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="trezo-card-header mb-[20px] md:mb-[25px] flex items-center justify-between">
                    ...
                    <a href="{{ route('dashboard.consultas-medicas.create') }}"
                ...
                <div class="trezo-card-content">
                        ...
                        <table class="w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th class="whitespace-nowrap uppercase text-[10px] font-bold tracking-[1px] ltr:text-left pt-0 pb-[12.5px] px-[20px] text-gray-500 dark:text-gray-400">Especialidad / Médico</th>
                                    <th class="whitespace-nowrap uppercase text-[10px] font-bold tracking-[1px] ltr:text-left pt-0 pb-[12.5px] px-[20px] text-gray-500 dark:text-gray-400">Motivo</th>
                                    <th class="whitespace-nowrap uppercase text-[10px] font-bold tracking-[1px] ltr:text-left pt-0 pb-[12.5px] px-[20px] text-gray-500 dark:text-gray-400">Acciones</th>
                                </tr>
                            </thead>
                            <tbody class="text-black dark:text-white">
                                ...
                                @php
                                    $miembroLabel = trim(implode(' ', array_filter([
                                        $c->hogarMiembro?->user?->persona?->nombres,
                                        $c->hogarMiembro?->user?->persona?->apellido_paterno,
                                        $c->hogarMiembro?->user?->persona?->apellido_materno,
                                    ]))) ?: ($c->hogarMiembro?->user?->name ?? '—');
                                    $espIcono = $c->especialidadMedica?->icono ?? 'stethoscope';
                                @endphp
                                <tr>
                                    <td class="ltr:text-left whitespace-nowrap px-[20px] py-[12.5px] ltr:first:pl-0 border-b border-primary-50 dark:border-[#172036] ltr:last:pr-0">
                                        <span class="block text-xs font-semibold text-black dark:text-white font-mono">
                                            {{ $c->fecha?->format('d/m/Y') ?? '—' }}
                                        </span>
                                        @if($c->hora)
                                        <span class="block text-xs text-gray-400 font-mono">{{ substr($c->hora, 0, 5) }}</span>
                                        @endif
                                    </td>
                                    <td class="ltr:text-left whitespace-nowrap px-[20px] py-[12.5px] border-b border-primary-50 dark:border-[#172036]">
                                        <span class="block text-xs font-semibold text-gray-700 dark:text-gray-300">{{ trim($miembroLabel) ?: '—' }}</span>
                                    </td>
                                    <td class="ltr:text-left whitespace-nowrap px-[20px] py-[12.5px] border-b border-primary-50 dark:border-[#172036]">
                                        @if($c->especialidadMedica)
                                        <span class="flex items-center gap-[5px] text-xs font-semibold text-gray-600 dark:text-gray-400">
                                            ...
                                            {{ $c->especialidadMedica->nombre }}
                                        </span>
                                        @endif
                                        @if($c->medico)
                                        <span class="block text-xs text-gray-500 dark:text-gray-400 mt-[2px]">
                                            Dr. {{ $c->medico->apellidos }}, {{ $c->medico->nombres }}
                                        </span>
                                        @endif
                                        @if(!$c->especialidadMedica && !$c->medico)
                                            <span class="text-xs text-gray-400">—</span>
                                        @endif
                                    </td>
                                    <td class="ltr:text-left px-[20px] py-[12.5px] border-b border-primary-50 dark:border-[#172036] max-w-[240px]">
                                        <span class="block text-xs text-gray-600 dark:text-gray-400 truncate">{{ $c->motivo ?: '—' }}</span>
                                    </td>
                                ...
                                @empty
                                <tr>
                                    <td colspan="5" class="text-center px-[20px] py-[50px] text-gray-500 dark:text-gray-400">
                    ...
                    <div class="pt-[12.5px] sm:flex sm:items-center justify-between">
                        <p class="!mb-0 text-xs">
                            Mostrando {{ $consultas->firstItem() }}–{{ $consultas->lastItem() }} de {{ $consultas->total() }} resultados
                        </p>
                        <ol class="mt-[10px] sm:mt-0">
                            <li class="inline-block mx-[1px]">
                                @if($consultas->onFirstPage())
                                    <span class="w-[31px] h-[31px] block leading-[29px] relative text-center rounded-md border border-gray-100 dark:border-[#172036] opacity-40 cursor-not-allowed">
                                        <span class="opacity-0">0</span><i class="material-symbols-outlined left-0 right-0 absolute top-1/2 -translate-y-1/2">chevron_left</i>
                                    </span>
                                @else
                                    <a href="{{ $consultas->previousPageUrl() }}" class="w-[31px] h-[31px] block leading-[29px] relative text-center rounded-md border border-gray-100 dark:border-[#172036] transition-all hover:bg-primary-500 hover:text-white hover:border-primary-500">
                                        <span class="opacity-0">0</span><i class="material-symbols-outlined left-0 right-0 absolute top-1/2 -translate-y-1/2">chevron_left</i>
                                    </a>
                                @endif
                            </li>
                            ...
                            <li class="inline-block mx-[1px]">
                                @if($consultas->hasMorePages())
                                    <a href="{{ $consultas->nextPageUrl() }}" class="w-[31px] h-[31px] block leading-[29px] relative text-center rounded-md border border-gray-100 dark:border-[#172036] transition-all hover:bg-primary-500 hover:text-white hover:border-primary-500">
                                        <span class="opacity-0">0</span><i class="material-symbols-outlined left-0 right-0 absolute top-1/2 -translate-y-1/2">chevron_right</i>
                                    </a>
                                @else
                                    <span class="w-[31px] h-[31px] block leading-[29px] relative text-center rounded-md border border-gray-100 dark:border-[#172036] opacity-40 cursor-not-allowed">
                                        <span class="opacity-0">0</span><i class="material-symbols-outlined left-0 right-0 absolute top-1/2 -translate-y-1/2">chevron_right</i>
