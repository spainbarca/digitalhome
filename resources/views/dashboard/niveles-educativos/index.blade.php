...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                ...
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">Niveles Educativos</li>
                </ol>
            </div>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-gray-100 dark:border-[#172036]">
                                    ...
                                    <th class="text-left py-[13px] px-[16px] text-sm font-semibold text-gray-500 dark:text-gray-400 w-[70px]">Orden</th>
                            ...
                            <tbody id="tablaBody">
                                ...
                                <tr class="border-b border-gray-100 dark:border-[#172036] hover:bg-gray-50 dark:hover:bg-[#15203c] transition-colors"
                                    ...
                                    <td class="py-[13px] px-[16px]">
                                        <i class="material-symbols-outlined !text-[22px] text-primary-500">{{ $n->icono ?? 'layers' }}</i>
                                    </td>
                                    <td class="py-[13px] px-[16px] text-sm text-gray-600 dark:text-gray-400 font-medium">{{ $n->orden }}</td>
                                    <td class="py-[13px] px-[16px] text-sm text-black dark:text-white font-medium">{{ $n->nombre }}</td>
                                    <td class="py-[13px] px-[16px] text-sm text-gray-600 dark:text-gray-400">{{ $n->descripcion ?? '—' }}</td>
                                    <td class="py-[13px] px-[16px]">
                                        @if($n->activo)
                                    ...
                                    <td class="py-[13px] px-[16px] text-right">
                                        ...
                                        <button type="button" onclick="eliminar('{{ $n->id }}', '{{ addslashes($n->nombre) }}', this.closest('tr'))"
                                ...
                                <tr id="filaVacia">
                                    <td colspan="6" class="text-center py-[50px] text-gray-500 dark:text-gray-400">
                                        <i class="material-symbols-outlined !text-[48px] text-gray-300 dark:text-gray-600 block mb-[10px]">layers</i>
        ...
        <!-- Modal -->
        <div id="modalNivel" class="hidden fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="relative bg-white dark:bg-[#0c1427] rounded-md w-full max-w-md mx-[16px] shadow-2xl z-10 max-h-[90vh] overflow-y-auto">
                ...
                <div class="flex items-center justify-between px-[25px] py-[18px] border-b border-gray-100 dark:border-[#172036] sticky top-0 bg-white dark:bg-[#0c1427] z-10">
                    <h5 class="!mb-0 text-[16px]" id="modalTitulo">Nuevo Nivel Educativo</h5>
                ...
                <div class="px-[25px] py-[20px] space-y-[16px]">
                    ...
                    <div>
                        ...
                        <input type="text" id="modalNombre" maxlength="100" placeholder="Ej: Primaria"
                    ...
                    <div>
                        ...
                        <textarea id="modalDescripcion" maxlength="255" rows="2" placeholder="Descripción opcional..."
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-[10px]">
                            <input type="text" id="modalIcono" maxlength="100" placeholder="Ej: layers"
                            ...
                            <div class="flex-shrink-0 w-[46px] h-[46px] rounded-md border border-gray-200 dark:border-[#172036] flex items-center justify-center bg-primary-50 dark:bg-[#15203c]">
                                <i class="material-symbols-outlined !text-[22px] text-primary-500" id="previewIcono">layers</i>
                            </div>
                        </div>
                    ...
                    <div>
                        <label class="mb-[8px] text-black dark:text-white font-medium block text-sm">Orden <span class="text-danger-500">*</span></label>
                        <input type="number" id="modalOrden" min="0" placeholder="Ej: 1"
                            class="h-[46px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[14px] block w-full outline-0 transition-all focus:border-primary-500 text-sm">
        ...
        <script>
            ...
            const storeUrl   = '{{ route('dashboard.niveles-educativos.store') }}';
            const baseUrl    = '{{ url('dashboard/niveles-educativos') }}';
            const iconDefault = 'layers';
            ...
            function abrirModalCrear() {
                ...
                document.getElementById('modalTitulo').textContent  = 'Nuevo Nivel Educativo';
                ...
                document.getElementById('modalOrden').value         = '';
            ...
            function abrirModalEditar(fila) {
                ...
                document.getElementById('modalTitulo').textContent  = 'Editar Nivel Educativo';
            ...
            async function guardar() {
                ...
                const payload = {
                    ...
                    orden:       parseInt(document.getElementById('modalOrden').value) || 0,
            ...
            function prependarFila(n) {
                ...
                tr.dataset.icono       = n.icono ?? '';
                tr.dataset.orden       = n.orden;
                tr.dataset.activo      = n.activo ? '1' : '0';
                tr.innerHTML = `
                    <td class="py-[13px] px-[16px]"><i class="material-symbols-outlined !text-[22px] text-primary-500">${esc(n.icono || iconDefault)}</i></td>
                    <td class="py-[13px] px-[16px] text-sm text-gray-600 font-medium">${n.orden}</td>
                    <td class="py-[13px] px-[16px] text-sm text-black font-medium">${esc(n.nombre)}</td>
                    <td class="py-[13px] px-[16px] text-sm text-gray-600">${esc(n.descripcion) || '—'}</td>
                    <td class="py-[13px] px-[16px]">${badgeActivo(n.activo)}</td>
                    <td class="py-[13px] px-[16px] text-right">${botonesHtml(n)}</td>`;
                document.getElementById('tablaBody').prepend(tr);
            }

            function actualizarFila(n) {
                const tr = document.querySelector(`#tablaBody tr[data-id="${n.id}"]`);
                if (!tr) return;
                tr.dataset.nombre      = n.nombre;
                tr.dataset.descripcion = n.descripcion ?? '';
                tr.dataset.icono       = n.icono ?? '';
                ...
                tds[0].innerHTML   = `<i class="material-symbols-outlined !text-[22px] text-primary-500">${esc(n.icono || iconDefault)}</i>`;
                tds[1].textContent = n.orden;
                tds[2].textContent = n.nombre;
                tds[3].textContent = n.descripcion || '—';
                tds[4].innerHTML   = badgeActivo(n.activo);
                tds[5].innerHTML   = `<div class="flex justify-end">${botonesHtml(n)}</div>`;
            }
            ...
            function mostrarFilaVacia() {
                ...
                tr.innerHTML = `<td colspan="6" class="text-center py-[50px] text-gray-500">
                    <i class="material-symbols-outlined !text-[48px] text-gray-300 block mb-[10px]">layers</i>
                    No hay niveles educativos registrados.</td>`;
                document.getElementById('tablaBody').appendChild(tr);
            }

            function mostrarErrores(errors) {
                const mapa = { nombre: 'errNombre', descripcion: 'errDescripcion', icono: 'errIcono', orden: 'errOrden' };
            ...
            function limpiarErrores() {
                ['errNombre', 'errDescripcion', 'errIcono', 'errOrden'].forEach(id => {
                    const el = document.getElementById(id);
                    el.textContent = '';
