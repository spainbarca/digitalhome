...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Editar — {{ $sector->nombre }}</title>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                ...
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.sectores.show', $sector) }}" class="transition-all hover:text-primary-500 truncate">{{ $sector->nombre }}</a>
                    </li>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="trezo-card-header bg-gray-50 dark:bg-[#15203c] mb-[25px] flex items-center justify-between -mx-[20px] md:-mx-[25px] -mt-[20px] md:-mt-[25px] p-[20px] md:p-[25px] rounded-t-md">
                    <h5 class="!mb-0">Editar: {{ $sector->nombre }}</h5>
                </div>

                <div class="trezo-card-content">
                    <form method="POST" action="{{ route('dashboard.sectores.update', $sector) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                            ...
                            <div>
                                ...
                                <input type="text" name="nombre" value="{{ old('nombre', $sector->nombre) }}" maxlength="120"
                            ...
                            <div>
                                ...
                                <div class="relative flex items-center gap-[10px]">
                                    <div class="w-[55px] h-[55px] flex-shrink-0 rounded-md border border-gray-200 dark:border-[#172036] bg-gray-50 dark:bg-[#15203c] flex items-center justify-center">
                                        <i id="iconPreview" class="{{ old('icono', $sector->icono ?? 'ri-building-line') }} text-[24px] text-primary-500"></i>
                                    </div>
                                    <input type="text" name="icono" id="iconoInput" value="{{ old('icono', $sector->icono) }}" maxlength="100"
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="descripcion" rows="3" maxlength="300"
                                    ...
                                    class="rounded-md text-black dark:text-white border {{ $errors->has('descripcion') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] py-[12px] block w-full outline-0 transition-all placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:border-primary-500 resize-none">{{ old('descripcion', $sector->descripcion) }}</textarea>
                            ...
                            <div class="flex items-center gap-[12px] pt-[6px]">
                                ...
                                <label class="relative cursor-pointer">
                                    <input type="checkbox" name="activo" value="1" id="activo"
                                        class="sr-only peer" {{ old('activo', $sector->activo) ? 'checked' : '' }}>
                        ...
                        <div class="mt-[25px] ltr:text-right rtl:text-left flex justify-end gap-[15px]">
                            <a href="{{ route('dashboard.sectores.show', $sector) }}"
