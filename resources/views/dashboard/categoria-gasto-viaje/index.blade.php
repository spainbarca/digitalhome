...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Categorías de Gasto de Viaje</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Categorías de Gasto de Viaje</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">Categorías de Gasto de Viaje</li>
                </ol>
            </div>
                        ...
                        <table class="w-full">
                            ...
                            <tbody id="tablaBody">
                                ...
                                <tr class="border-b border-gray-100 dark:border-[#172036] hover:bg-gray-50 dark:hover:bg-[#15203c] transition-colors"
                                    ...
                                    data-icono="{{ $e->icono ?? '' }}"
                                    data-color="{{ $e->color ?? '#6b7280' }}"
                                    data-activo="{{ $e->activo ? '1' : '0' }}">
                                    <td class="py-[13px] px-[16px]">
                                        <i class="material-symbols-outlined !text-[22px] text-primary-500">{{ $e->icono ?? 'luggage' }}</i>
                                    </td>
                                    <td class="py-[13px] px-[16px] text-sm text-black dark:text-white font-medium">{{ $e->nombre }}</td>
                                    <td class="py-[13px] px-[16px]">
                                        @php $color = $e->color ?? '#6b7280'; @endphp
                                        <span class="inline-flex items-center gap-[6px] text-xs font-medium px-[8px] py-[2px] rounded-[100px] border text-white"
                                ...
                                <tr id="filaVacia">
                                    <td colspan="6" class="text-center py-[50px] text-gray-500 dark:text-gray-400">
                                        <i class="material-symbols-outlined !text-[48px] text-gray-300 dark:text-gray-600 block mb-[10px]">attach_money</i>
                                        No hay categorías de gasto de viaje registradas.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
        ...
        <div id="modalCategoria" class="hidden fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="relative bg-white dark:bg-[#0c1427] rounded-md w-full max-w-md mx-[16px] shadow-2xl z-10 max-h-[90vh] overflow-y-auto">
                ...
                <div class="flex items-center justify-between px-[25px] py-[18px] border-b border-gray-100 dark:border-[#172036] sticky top-0 bg-white dark:bg-[#0c1427] z-10">
                    <h5 class="!mb-0 text-[16px]" id="modalTitulo">Nuevo Estado de Reserva</h5>
                ...
                <div class="px-[25px] py-[20px] space-y-[16px]">
                    ...
                    <div>
                        ...
                        <input type="text" id="modalNombre" maxlength="255" placeholder="Ej: Alojamiento"
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-[10px]">
                            <input type="text" id="modalIcono" maxlength="100" placeholder="Ej: attach_money"
                            ...
                            <div class="flex-shrink-0 w-[46px] h-[46px] rounded-md border border-gray-200 dark:border-[#172036] flex items-center justify-center bg-primary-50 dark:bg-[#15203c]">
                                <i class="material-symbols-outlined !text-[22px] text-primary-500" id="previewIcono">attach_money</i>
                            </div>
                        </div>
        ...
        <script>
            ...
            const storeUrl    = '{{ route('dashboard.categoria-gasto-viaje.store') }}';
            const baseUrl     = '{{ url('dashboard/categoria-gasto-viaje') }}';
            const iconDefault = 'attach_money';
            ...
            function abrirModalCrear() {
                ...
                document.getElementById('modalTitulo').textContent     = 'Nueva Categoría de Gasto de Viaje';
            ...
            function abrirModalEditar(fila) {
                ...
                document.getElementById('modalTitulo').textContent     = 'Editar Categoría de Gasto de Viaje';
                ...
                document.getElementById('modalColor').value            = fila.dataset.color || '#22c55e';
                document.getElementById('previewColorHex').textContent = fila.dataset.color || '#22c55e';
            ...
            function cerrarModal() { document.getElementById('modalCategoria').classList.add('hidden'); }
            ...
            function prependarFila(e) {
                ...
                tr.dataset.color       = e.color ?? '#6b7280';
            ...
            function actualizarFila(e) {
                ...
                tds[2].innerHTML   = badgeColor(e.color);
                tds[3].textContent = e.descripcion || '—';
            ...
            function mostrarFilaVacia() {
                ...
                tr.innerHTML = `<td colspan="6" class="text-center py-[50px] text-gray-500">
                    <i class="material-symbols-outlined !text-[48px] text-gray-300 block mb-[10px]">attach_money</i>
                    No hay categorías de gasto de viaje registradas.</td>`;
                document.getElementById('tablaBody').appendChild(tr);
            }

            function mostrarErrores(errors) {
                const mapa = { nombre: 'errNombre', descripcion: 'errDescripcion', color: 'errColor', icono: 'errIcono' };
            ...
            function limpiarErrores() {
                ['errNombre', 'errDescripcion', 'errColor', 'errIcono'].forEach(id => {
                    const el = document.getElementById(id);
                    el.textContent = '';
