    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Tipos de Inmueble</h5>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-gray-100 dark:border-[#172036]">
                                    <th class="text-left py-[13px] px-[16px] text-sm font-semibold text-gray-500 dark:text-gray-400 w-[56px]">Ícono</th>
                                    <th class="text-left py-[13px] px-[16px] text-sm font-semibold text-gray-500 dark:text-gray-400">Nombre</th>
                                    <th class="text-left py-[13px] px-[16px] text-sm font-semibold text-gray-500 dark:text-gray-400 w-[110px]">Propiedades</th>
                            ...
                            <tbody id="tablaBody">
                                ...
                                <tr class="border-b border-gray-100 dark:border-[#172036] hover:bg-gray-50 dark:hover:bg-[#15203c] transition-colors"
                                    ...
                                    data-count="{{ $t->propiedades_inmueble_count }}">
                                    <td class="py-[13px] px-[16px]">
                                        <i class="material-symbols-outlined !text-[22px] text-primary-500">{{ $t->icono ?? 'home' }}</i>
                                    </td>
                                    ...
                                    <td class="py-[13px] px-[16px] text-right">
                                        ...
                                        <button type="button" onclick="eliminar('{{ $t->id }}', '{{ addslashes($t->nombre) }}', {{ $t->propiedades_inmueble_count }}, this.closest('tr'))"
                                ...
                                <tr id="filaVacia">
                                    <td colspan="5" class="text-center py-[50px] text-gray-500 dark:text-gray-400">
                                        <i class="material-symbols-outlined !text-[48px] text-gray-300 dark:text-gray-600 block mb-[10px]">home_work</i>
        ...
        <div id="modal" class="hidden fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="relative bg-white dark:bg-[#0c1427] rounded-md w-full max-w-sm mx-[16px] shadow-2xl z-10">
                ...
                <div class="flex items-center justify-between px-[25px] py-[18px] border-b border-gray-100 dark:border-[#172036]">
                    <h5 class="!mb-0 text-[16px]" id="modalTitulo">Nuevo Tipo de Inmueble</h5>
                ...
                <div class="px-[25px] py-[20px] space-y-[16px]">
                    ...
                    <div>
                        ...
                        <input type="text" id="modalNombre" maxlength="80" placeholder="Ej: Departamento"
                    ...
                    <div>
                        <label class="mb-[8px] text-black dark:text-white font-medium block text-sm">
                            Ícono
                            <span class="text-xs text-gray-400 font-normal ml-[4px]">(material-symbols-outlined)</span>
                        </label>
                        <div class="flex items-center gap-[10px]">
                            <input type="text" id="modalIcono" maxlength="60" placeholder="Ej: apartment"
                            ...
                            <div class="flex-shrink-0 w-[46px] h-[46px] rounded-md border border-gray-200 dark:border-[#172036] flex items-center justify-center bg-primary-50 dark:bg-[#15203c]">
                                <i class="material-symbols-outlined !text-[22px] text-primary-500" id="previewIcono">home</i>
                            </div>
                        </div>
        ...
        <script>
            ...
            const STORE   = '{{ route('dashboard.tipo-inmueble.store') }}';
            const BASE    = '{{ url('dashboard/tipo-inmueble') }}';
            let   editMode = false;
            let   editId   = null;
            ...
            function abrirModalCrear() {
                ...
                document.getElementById('modalTitulo').textContent = 'Nuevo Tipo de Inmueble';
                document.getElementById('modalId').value     = '';
                document.getElementById('modalNombre').value = '';
                document.getElementById('modalIcono').value  = '';
                document.getElementById('previewIcono').textContent = 'home';
            ...
            function abrirModalEditar(tr) {
                ...
                document.getElementById('modalTitulo').textContent = 'Editar Tipo de Inmueble';
                document.getElementById('modalId').value     = tr.dataset.id;
                document.getElementById('modalNombre').value = tr.dataset.nombre;
                document.getElementById('modalIcono').value  = tr.dataset.icono;
                document.getElementById('previewIcono').textContent = tr.dataset.icono || 'home';
            ...
            function actualizarPreview() {
                ...
                document.getElementById('previewIcono').textContent = val || 'home';
            }
            ...
            async function eliminar(id, nombre, count, tr) {
                if (count > 0) {
                    alert(`No se puede eliminar «${nombre}» porque tiene ${count} propiedad(es) asociada(s).`);
                    return;
                }
                ...
                try {
                    ...
                    if (resp.status === 422) {
                        const json = await resp.json();
                        alert(json.message ?? 'No se puede eliminar.');
                        return;
                    }
            ...
            function botonesAccion(u) {
                return `
                    ...
                    <button type="button" onclick="eliminar('${u.id}','${esc(u.nombre)}',${u.propiedades_inmueble_count ?? 0},this.closest('tr'))"
            ...
            function insertarFila(u) {
                ...
                tr.dataset.count  = 0;
                tr.innerHTML = `
                    <td class="py-[13px] px-[16px]"><i class="material-symbols-outlined !text-[22px] text-primary-500">${esc(u.icono || 'home')}</i></td>
            ...
            function actualizarFila(u) {
                ...
                tds[0].innerHTML  = `<i class="material-symbols-outlined !text-[22px] text-primary-500">${esc(u.icono || 'home')}</i>`;
                tds[1].textContent = u.nombre;
                tds[3].innerHTML  = badgeActivo(u.activo);
                tds[4].innerHTML  = `<div class="flex justify-end">${botonesAccion(u)}</div>`;
            }

            function mostrarFilaVacia() {
                const tr = document.createElement('tr');
                tr.id = 'filaVacia';
                tr.innerHTML = `<td colspan="5" class="text-center py-[50px] text-gray-500 dark:text-gray-400">
                    <i class="material-symbols-outlined !text-[48px] text-gray-300 dark:text-gray-600 block mb-[10px]">home_work</i>
                    No hay tipos de inmueble registrados.</td>`;
                document.getElementById('tablaBody').appendChild(tr);
            }

            /* ── Errores ────────────────────────────────────────── */
            function mostrarErrores(errs) {
                const map = { nombre: 'errNombre', icono: 'errIcono' };
                Object.entries(map).forEach(([k, id]) => {
                    const el = document.getElementById(id);
                    if (errs[k]) { el.textContent = errs[k][0]; el.classList.remove('hidden'); }
                });
            }
            function limpiarErrores() {
                ['errNombre', 'errIcono'].forEach(id => {
                    const el = document.getElementById(id);
