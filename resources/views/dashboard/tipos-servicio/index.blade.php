    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Tipos de Servicio</h5>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-gray-100 dark:border-[#172036]">
                                    ...
                                    <th class="text-left py-[13px] px-[16px] text-sm font-semibold text-gray-500 dark:text-gray-400 whitespace-nowrap">Unidad de Medida</th>
                            ...
                            <tbody id="tablaBody">
                                ...
                                <tr class="border-b border-gray-100 dark:border-[#172036] hover:bg-gray-50 dark:hover:bg-[#15203c] transition-colors"
                                    ...
                                    data-unidad-id="{{ $t->unidad_medida_id ?? '' }}"
                                    data-unidad-nombre="{{ $t->unidadMedida?->nombre ?? '' }}"
                                    data-unidad-simbolo="{{ $t->unidadMedida?->simbolo ?? '' }}">
                                    <td class="py-[13px] px-[16px]">
                                        <i class="material-symbols-outlined !text-[22px] text-primary-500">{{ $t->icono ?? 'home_repair_service' }}</i>
                                    </td>
                                    ...
                                    <td class="py-[13px] px-[16px] text-sm text-gray-600 dark:text-gray-400">
                                        @if($t->unidadMedida)
                                            {{ $t->unidadMedida->nombre }}
                                            <span class="text-xs text-gray-400 font-mono ml-[4px]">({{ $t->unidadMedida->simbolo }})</span>
                                        @else
                                    ...
                                    <td class="py-[13px] px-[16px] text-right">
                                        ...
                                        <button type="button"
                                            onclick="eliminar('{{ $t->id }}', '{{ addslashes($t->nombre) }}', this.closest('tr'))"
        ...
        <div id="modalTipoServicio" class="hidden fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="relative bg-white dark:bg-[#0c1427] rounded-md w-full max-w-md mx-[16px] shadow-2xl z-10">
                ...
                <div class="flex items-center justify-between px-[25px] py-[18px] border-b border-gray-100 dark:border-[#172036]">
                    <h5 class="!mb-0 text-[16px]" id="modalTipoTitulo">Nuevo Tipo de Servicio</h5>
                ...
                <div class="px-[25px] py-[20px] space-y-[16px]">
                    ...
                    <div>
                        ...
                        <input type="text" id="modalNombre" maxlength="100" placeholder="Ej: Electricidad"
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-[10px]">
                            <input type="text" id="modalIcono" maxlength="60" placeholder="Ej: bolt"
                            ...
                            <div class="flex-shrink-0 w-[46px] h-[46px] rounded-md border border-gray-200 dark:border-[#172036] flex items-center justify-center bg-primary-50 dark:bg-[#15203c]">
                                <i class="material-symbols-outlined !text-[22px] text-primary-500" id="previewIcono">home_repair_service</i>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <select id="modalUnidadMedidaId"
                            class="h-[46px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[14px] block w-full outline-0 transition-all focus:border-primary-500 text-sm cursor-pointer">
                            <option value="">Seleccionar unidad...</option>
                            @foreach($unidades as $u)
                            <option value="{{ $u->id }}">{{ $u->nombre }} ({{ $u->simbolo }})</option>
                            @endforeach
                        </select>
                        <p id="errUnidadMedidaId" class="hidden text-xs text-danger-500 mt-[5px]"></p>
                    </div>
        ...
        <script>
            ...
            const storeUrl    = '{{ route('dashboard.tipos-servicio.store') }}';
            const baseUrl     = '{{ url('dashboard/tipos-servicio') }}';
            ...
            function abrirModalCrear() {
                ...
                document.getElementById('previewIcono').textContent  = 'home_repair_service';
                document.getElementById('modalUnidadMedidaId').value = '';
                document.getElementById('modalActivo').checked       = true;
                limpiarErrores();
                document.getElementById('modalTipoServicio').classList.remove('hidden');
            ...
            function abrirModalEditar(fila) {
                ...
                document.getElementById('previewIcono').textContent  = fila.dataset.icono || 'home_repair_service';
                document.getElementById('modalUnidadMedidaId').value = fila.dataset.unidadId;
            ...
            function cerrarModal() {
                document.getElementById('modalTipoServicio').classList.add('hidden');
            }

            function actualizarPreviewIcono() {
                const val = document.getElementById('modalIcono').value.trim();
                document.getElementById('previewIcono').textContent = val || 'home_repair_service';
            }
            ...
            async function guardar() {
                ...
                const payload = {
                    ...
                    unidad_medida_id: document.getElementById('modalUnidadMedidaId').value,
            ...
            function unidadTexto(data) {
                const u = data.unidad_medida;
                if (!u) return '<span class="text-gray-400">—</span>';
                return `${esc(u.nombre)} <span class="text-xs text-gray-400 font-mono ml-[4px]">(${esc(u.simbolo)})</span>`;
            }

            function botonesFila(t) {
                const n = esc(t.nombre);
                return `
                    ...
                    <button type="button" onclick="eliminar('${t.id}','${n}',this.closest('tr'))"
            ...
            function prependarFila(t) {
                ...
                tr.dataset.unidadId     = t.unidad_medida_id ?? '';
                tr.dataset.unidadNombre = u?.nombre ?? '';
                tr.dataset.unidadSimbolo = u?.simbolo ?? '';
                tr.innerHTML = `
                    <td class="py-[13px] px-[16px]"><i class="material-symbols-outlined !text-[22px] text-primary-500">${esc(t.icono || 'home_repair_service')}</i></td>
                    <td class="py-[13px] px-[16px] text-sm text-black font-medium">${esc(t.nombre)}</td>
                    <td class="py-[13px] px-[16px] text-sm text-gray-600">${unidadTexto(t)}</td>
                    <td class="py-[13px] px-[16px]">${badgeActivo(t.activo)}</td>
                    <td class="py-[13px] px-[16px] text-right">${botonesFila(t)}</td>`;
            ...
            function actualizarFila(t) {
                ...
                tds[0].innerHTML   = `<i class="material-symbols-outlined !text-[22px] text-primary-500">${esc(t.icono || 'home_repair_service')}</i>`;
                tds[1].textContent = t.nombre;
                tds[2].innerHTML   = unidadTexto(t);
                tds[3].innerHTML   = badgeActivo(t.activo);
                tds[4].innerHTML   = `<div class="flex justify-end">${botonesFila(t)}</div>`;
            }
            ...
            function mostrarErrores(errors) {
                const mapa = {
                    nombre:           'errNombre',
                    icono:            'errIcono',
                    unidad_medida_id: 'errUnidadMedidaId',
                };
            ...
            function limpiarErrores() {
                ['errNombre', 'errIcono', 'errUnidadMedidaId'].forEach(id => {
                    const el = document.getElementById(id);
                    el.textContent = '';
