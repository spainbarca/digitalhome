...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Subir Documento Médico</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Subir Documento Médico</h5>
            ...
            <form method="POST" action="{{ route('dashboard.documentos-medicos.store') }}" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" name="titulo" value="{{ old('titulo') }}" maxlength="200"
                                    placeholder="Ej: Resultado de análisis de sangre"
                            ...
                            <div>
                                ...
                                <select name="hogar_miembro_id" id="selMiembro" class="w-full">
                                    ...
                                    @foreach($miembros as $m)
                                        ...
                                        <option value="{{ $m->id }}" {{ old('hogar_miembro_id') == $m->id ? 'selected' : '' }}>{{ trim($lbl) }}</option>
                                    @endforeach
                                </select>
                                ...
                                <select name="tipo_documento_medico_id" id="selTipo" class="w-full">
                                    ...
                                    @foreach($tiposDocumento as $td)
                                        <option value="{{ $td->id }}" data-icono="{{ $td->icono ?? 'description' }}"
                                            {{ old('tipo_documento_medico_id') == $td->id ? 'selected' : '' }}>
                                            {{ $td->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <!-- Consulta vinculada (carga via AJAX al seleccionar miembro) -->
                            <div>
        ...
        <script>
            ...
            // Pre-selección inicial: viene de ?consulta_medica_id=... (desde show de consulta)
            // o de old() al haber error de validación
            const initMiembro  = '{{ $consultaPresel?->hogar_miembro_id ?? old('hogar_miembro_id') ?? '' }}';
            const initConsulta = '{{ $consultaPresel?->id ?? old('consulta_medica_id') ?? '' }}';
            ...
            $(document).ready(function() {
                ...
                // Pre-cargar si hay miembro inicial (pre-sel de consulta o vuelta por error de validación)
                if (initMiembro) {
                    $('#selMiembro').val(initMiembro).trigger('change');
                    cargarConsultas(initMiembro, initConsulta);
                }

                // Reaccionar solo a interacción del usuario (select2:select / select2:unselect)
                // así evitamos disparar la carga al setear el valor programáticamente arriba
