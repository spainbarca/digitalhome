...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Operadores de Viaje</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Operadores de Viaje</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">Operadores de Viaje</li>
                </ol>
            </div>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="flex flex-wrap items-center gap-[12px]">
                    ...
                    <!-- Búsqueda -->
                    <form method="GET" action="{{ route('dashboard.operadores-viaje.index') }}"
                    ...
                    <div class="flex items-center gap-[6px] flex-wrap">
                        ...
                        <a href="{{ route('dashboard.operadores-viaje.index', array_merge(request()->except('tipo_id'), ['search' => $search])) }}"
                        ...
                        @foreach($tipos as $t)
                        <a href="{{ route('dashboard.operadores-viaje.index', array_merge(request()->except('tipo_id'), ['search' => $search, 'tipo_id' => $t->id])) }}"
                           class="inline-flex items-center gap-[5px] px-[12px] py-[6px] rounded-full text-xs font-medium transition-all border
                           {{ $tipoId == $t->id ? 'bg-primary-500 text-white border-primary-500' : 'bg-white dark:bg-[#0c1427] text-gray-600 dark:text-gray-300 border-gray-200 dark:border-[#172036] hover:border-primary-400' }}">
                            <i class="material-symbols-outlined !text-[13px]">{{ $t->icono ?? 'flight' }}</i>
                            {{ $t->nombre }}
                        </a>
                        @endforeach
                    </div>
                    ...
                    <a href="{{ route('dashboard.operadores-viaje.create') }}"
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] p-[40px] rounded-md text-center">
                <i class="material-symbols-outlined !text-[56px] text-gray-300 dark:text-gray-600 block mb-[12px]">flight</i>
                <p class="text-gray-500 dark:text-gray-400">
                    @if($search || $tipoId)
                        No se encontraron operadores con los filtros aplicados.
                    @else
                        No hay operadores de viaje registrados aún.
                    @endif
                </p>
                @if(!$search && !$tipoId)
                <a href="{{ route('dashboard.operadores-viaje.create') }}"
                ...
                <div class="trezo-card-content">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-[25px]">
                        ...
                        @php
                            ...
                            $gradiente = $colores[abs(crc32($op->id)) % count($colores)];
                            $nombre    = $op->nombre_comercial_resuelto ?? '—';
                            $inicial   = strtoupper(mb_substr($nombre, 0, 1));
                            $tipo      = $op->tipoOperadorViaje;
                            $logoSrc   = $op->logo_resuelto;
                        @endphp
                        <div class="bg-white dark:bg-[#0c1427] p-[10px] rounded-md shadow-sm hover:shadow-md transition-shadow">
                            ...
                            <div class="text-center -mt-[34px]">
                                ...
                                @if($op->sigla_resuelta)
                                <span class="block text-xs font-mono font-semibold text-primary-500 truncate px-[8px] mb-[2px]">{{ $op->sigla_resuelta }}</span>
                                @endif
                                ...
                                <div class="flex items-center justify-center gap-[6px] mb-[10px] flex-wrap">
                                    ...
                                    <span class="inline-flex items-center gap-[3px] text-xs text-gray-500 dark:text-gray-400">
                                        <i class="material-symbols-outlined !text-[13px]">{{ $tipo->icono ?? 'flight' }}</i>
                                        {{ $tipo->nombre }}
                                    </span>
                                    @endif
                                    @if($op->codigo_iata)
                                    <span class="inline-flex items-center gap-[3px] text-xs text-gray-400 dark:text-gray-500 font-mono">
                                        <i class="material-symbols-outlined !text-[13px]">confirmation_number</i>
                                        {{ $op->codigo_iata }}
                                    </span>
                                    @endif
                                </div>
                                ...
                                <div class="flex items-center justify-center gap-[8px] pb-[4px]">
                                    ...
                                    <a href="{{ route('dashboard.operadores-viaje.edit', $op) }}"
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="sm:flex sm:items-center justify-between">
                    <p class="!mb-0 text-sm text-gray-500 dark:text-gray-400">
                        Mostrando {{ $operadores->firstItem() }}–{{ $operadores->lastItem() }} de {{ $operadores->total() }} operadores
                    </p>
