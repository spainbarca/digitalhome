...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $operadoresViaje->nombre_comercial_resuelto ?? 'Operador de Viaje' }}</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="rounded-md overflow-hidden h-[180px] md:h-[220px] relative z-0 mb-0">
                @if($op->banner_url)
                    <img src="{{ $op->banner_url }}" alt="{{ $nombre }}"
                ...
                <div class="absolute top-[12px] right-[12px] flex gap-[8px]">
                    ...
                    <form method="POST" action="{{ route('dashboard.operadores-viaje.destroy', $op) }}"
                        onsubmit="return confirm('¿Eliminar este operador de viaje?')">
                        @csrf @method('DELETE')
                        <button type="submit"
                ...
                <div class="lg:col-span-1 space-y-[25px]">
                    ...
                    <div class="trezo-card bg-white dark:bg-[#0c1427] p-[25px] rounded-md text-center">
                        ...
                        @if($op->sigla_resuelta)
                            <span class="inline-block text-xs font-mono font-semibold px-[8px] py-[2px] rounded bg-gray-100 dark:bg-[#15203c] text-gray-500 dark:text-gray-400 mb-[8px]">{{ $op->sigla_resuelta }}</span>
                        @endif
                    ...
                    <div class="trezo-card bg-white dark:bg-[#0c1427] p-[25px] rounded-md">
                        ...
                        <ul class="space-y-[12px]">
                            <li class="flex items-start gap-[10px] text-sm">
                                <i class="material-symbols-outlined !text-[16px] text-gray-400 mt-[1px] flex-shrink-0">confirmation_number</i>
                                <div>
                                    <p class="text-xs text-gray-400 mb-[2px]">Código IATA</p>
                                    <span class="text-black dark:text-white font-mono font-medium">{{ $op->codigo_iata }}</span>
                                </div>
                            </li>
                        </ul>
                    </div>
                        ...
                        <div class="border-b border-gray-100 dark:border-[#172036] px-[20px] md:px-[25px] pt-[20px] md:pt-[25px]">
                            <div class="flex gap-[0px] overflow-x-auto" id="tabsNav">
                                <button type="button"
                                    ...
                                    data-tab="reservas">
                                    <i class="material-symbols-outlined !text-[16px] align-middle mr-[4px]">luggage</i>
                                    Reservas
                                </button>
                            </div>
                        </div>

                        <!-- Tab: Reservas (preparado, tabla reservas pendiente) -->
                        <div id="tab-reservas" class="p-[20px] md:p-[25px]">
                            <div class="text-center py-[40px] text-gray-400 dark:text-gray-500">
                                <i class="material-symbols-outlined !text-[40px] block mb-[8px]">luggage</i>
                                <p class="text-sm">Las reservas estarán disponibles próximamente.</p>
                            </div>
                        </div>
