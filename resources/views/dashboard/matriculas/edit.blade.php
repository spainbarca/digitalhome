    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Editar Matrícula</h5>
            ...
            <form method="POST" action="{{ route('dashboard.matriculas.update', $matricula) }}">
                @csrf
                @method('PUT')
                            ...
                            <select name="hogar_miembro_id" id="selMiembro" class="w-full">
                                ...
                                @foreach($miembros as $m)
                                    ...
                                    <option value="{{ $m->id }}" data-avatar="{{ $avatarM }}"
                                        {{ old('hogar_miembro_id', $matricula->hogar_miembro_id) == $m->id ? 'selected' : '' }}>{{ $nombreM }}</option>
                                @endforeach
                            </select>
                            ...
                            <select name="institucion_educativa_id" id="selInstitucion" class="w-full">
                                ...
                                @foreach($instituciones as $inst)
                                    ...
                                    <option value="{{ $inst->id }}"
                                        ...
                                        {{ old('institucion_educativa_id', $matricula->institucion_educativa_id) == $inst->id ? 'selected' : '' }}>
                ...
                <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[20px] p-[20px] md:p-[25px] rounded-md">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[20px] md:gap-[25px]">
                            ...
                            <select name="nivel_educativo_id" id="selNivel" class="w-full">
                                ...
                                @foreach($niveles as $niv)
                                    <option value="{{ $niv->id }}" data-icono="{{ $niv->icono ?? 'layers' }}"
                                        {{ old('nivel_educativo_id', $matricula->nivel_educativo_id) == $niv->id ? 'selected' : '' }}>{{ $niv->nombre }}</option>
                                @endforeach
                            </select>
                            ...
                            <select name="turno_educativo_id" id="selTurno" class="w-full">
                                ...
                                @foreach($turnos as $turno)
                                    <option value="{{ $turno->id }}" data-icono="{{ $turno->icono ?? 'schedule' }}"
                                        {{ old('turno_educativo_id', $matricula->turno_educativo_id) == $turno->id ? 'selected' : '' }}>{{ $turno->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="año_lectivo" value="{{ old('año_lectivo', $matricula->año_lectivo) }}" placeholder="Ej: 2026"
                                class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                        ...
                        <div>
                            ...
                            <input type="text" name="grado_ciclo" value="{{ old('grado_ciclo', $matricula->grado_ciclo) }}" placeholder="Ej: 3° Primaria"
                                class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                ...
                <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[20px] p-[20px] md:p-[25px] rounded-md">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                        <div>
                            ...
                            <input type="text" name="nombre_tutor" value="{{ old('nombre_tutor', $matricula->nombre_tutor) }}" placeholder="Nombre completo del tutor"
                                class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                        ...
                        <div>
                            ...
                            <input type="text" name="telefono_tutor" value="{{ old('telefono_tutor', $matricula->telefono_tutor) }}" placeholder="Ej: +51 999 000 111"
                                class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                ...
                <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[20px] p-[20px] md:p-[25px] rounded-md">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[20px] md:gap-[25px]">
                        <div>
                            ...
                            <input type="number" name="costo_mensual" value="{{ old('costo_mensual', $matricula->costo_mensual) }}" step="0.01" min="0"
                                class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                        ...
                        <div>
                            ...
                            <select name="moneda_id" id="selMoneda" class="w-full">
                                ...
                                @foreach($monedas as $mon)
                                    <option value="{{ $mon->id }}" {{ old('moneda_id', $matricula->moneda_id) == $mon->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="dia_vencimiento_pago" value="{{ old('dia_vencimiento_pago', $matricula->dia_vencimiento_pago) }}" min="1" max="31"
                                class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                        </div>
                        <div>
                            <label class="mb-[10px] text-black dark:text-white font-medium block">Fecha de inicio</label>
                            <input type="date" name="fecha_inicio" value="{{ old('fecha_inicio', $matricula->fecha_inicio?->format('Y-m-d')) }}"
                                class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                        </div>
                        <div>
                            <label class="mb-[10px] text-black dark:text-white font-medium block">Fecha de fin</label>
                            <input type="date" name="fecha_fin" value="{{ old('fecha_fin', $matricula->fecha_fin?->format('Y-m-d')) }}"
                                class="h-[55px] rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                        </div>
                    </div>
                </div>
                            ...
                            <select name="estado_matricula_id" id="selEstado" class="w-full">
                                ...
                                @foreach($estados as $est)
                                    <option value="{{ $est->id }}"
                                        ...
                                        {{ old('estado_matricula_id', $matricula->estado_matricula_id) == $est->id ? 'selected' : '' }}>{{ $est->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <textarea name="notas" rows="3" maxlength="5000"
                                class="rounded-md text-black dark:text-white border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[17px] py-[13px] block w-full outline-0 focus:border-primary-500">{{ old('notas', $matricula->notas) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-[15px] mb-[25px]">
                    <a href="{{ route('dashboard.matriculas.show', $matricula) }}"
