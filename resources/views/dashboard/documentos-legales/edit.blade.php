    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Editar Documento Legal</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.documentos-legales.show', $documentoLegal) }}" class="transition-all hover:text-primary-500 truncate max-w-[200px] inline-block">
                            {{ Str::limit($documentoLegal->titulo, 30) }}
                        </a>
                    </li>
            ...
            @php
                $tipoActual = $documentoLegal->tipoDocumentoLegal;
                $requiereVenc = old('tipo_documento_legal_id')
                    ? ($tipos->firstWhere('id', old('tipo_documento_legal_id'))?->requiere_vencimiento ?? false)
                    : ($tipoActual?->requiere_vencimiento ?? false);
            @endphp

            <form method="POST" action="{{ route('dashboard.documentos-legales.update', $documentoLegal) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                                ...
                                <select name="tipo_documento_legal_id" id="selTipo" class="w-full">
                                    ...
                                    @foreach($tipos as $t)
                                        ...
                                        <option value="{{ $t->id }}"
                                            ...
                                            {{ old('tipo_documento_legal_id', $documentoLegal->tipo_documento_legal_id) == $t->id ? 'selected' : '' }}>
                                            {{ $t->nombre }} — {{ $cat }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="estado_documento_legal_id" id="selEstado" class="w-full">
                                    ...
                                    @foreach($estados as $e)
                                        <option value="{{ $e->id }}" data-color="{{ $e->color }}"
                                            {{ old('estado_documento_legal_id', $documentoLegal->estado_documento_legal_id) == $e->id ? 'selected' : '' }}>
                                            {{ $e->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="trezo-card-content">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" name="titulo" value="{{ old('titulo', $documentoLegal->titulo) }}" maxlength="255"
                            ...
                            <div>
                                ...
                                <input type="text" name="numero_documento" value="{{ old('numero_documento', $documentoLegal->numero_documento) }}" maxlength="100"
                            ...
                            <div>
                                ...
                                <input type="date" name="fecha_emision"
                                    value="{{ old('fecha_emision', $documentoLegal->fecha_emision?->format('Y-m-d')) }}"
                            ...
                            <div id="bloqueVencimiento" class="{{ $requiereVenc ? '' : 'hidden' }}">
                                <label class="mb-[10px] text-black dark:text-white font-medium block">
                                    Fecha de Vencimiento <span class="text-danger-500">*</span>
                                </label>
                                <input type="date" name="fecha_vencimiento"
                                    value="{{ old('fecha_vencimiento', $documentoLegal->fecha_vencimiento?->format('Y-m-d')) }}"
                            ...
                            <div>
                                ...
                                <select name="hogar_miembro_id" id="selMiembro" class="w-full">
                                    ...
                                    @foreach($miembros as $m)
                                        ...
                                        <option value="{{ $m->id }}" {{ old('hogar_miembro_id', $documentoLegal->hogar_miembro_id) == $m->id ? 'selected' : '' }}>{{ $lbl }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="propiedad_inmueble_id" id="selPropiedad" class="w-full">
                                    ...
                                    @foreach($propiedades as $prop)
                                        <option value="{{ $prop->id }}" {{ old('propiedad_inmueble_id', $documentoLegal->propiedad_inmueble_id) == $prop->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="entidad_legal_id" id="selEntidad" class="w-full">
                                    ...
                                    @foreach($entidades as $ent)
                                        <option value="{{ $ent->id }}" {{ old('entidad_legal_id', $documentoLegal->entidad_legal_id) == $ent->id ? 'selected' : '' }}>
                                            {{ $ent->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                            ...
                            <div class="sm:col-span-2">
                                @if($documentoLegal->file_path)
                                    @php
                                        $ext = strtolower(pathinfo($documentoLegal->file_path, PATHINFO_EXTENSION));
                                        $isImage = in_array($ext, ['jpg','jpeg','png','webp','gif']);
                                    @endphp
                                    <div class="mb-[16px] p-[16px] rounded-md bg-gray-50 dark:bg-[#15203c] border border-gray-200 dark:border-[#172036] flex items-center gap-[14px]">
                                        @if($isImage)
                                            <img src="{{ Storage::url($documentoLegal->file_path) }}" alt="Archivo actual"
                                                class="w-[60px] h-[60px] rounded-md object-cover border border-gray-200 dark:border-[#172036]">
                                        @else
                                            <div class="w-[60px] h-[60px] rounded-md bg-danger-100 dark:bg-danger-900 flex items-center justify-center flex-shrink-0">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-medium text-black dark:text-white truncate">
                                                {{ basename($documentoLegal->file_path) }}
                                            </p>
                                            <p class="text-xs text-gray-400 uppercase mt-[2px]">Archivo actual · {{ strtoupper($ext) }}</p>
                                        </div>
                                        <a href="{{ Storage::url($documentoLegal->file_path) }}" target="_blank"
                                            class="text-primary-500 hover:text-primary-400 flex-shrink-0">
                                ...
                                <label class="mb-[10px] text-black dark:text-white font-medium block">
                                    {{ $documentoLegal->file_path ? 'Reemplazar Archivo' : 'Archivo' }}
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="notas" rows="3" maxlength="2000"
                                    ...
                                    class="rounded-md text-black dark:text-white border {{ $errors->has('notas') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] py-[14px] block w-full outline-0 placeholder:text-gray-400 focus:border-primary-500 resize-none transition-all">{{ old('notas', $documentoLegal->notas) }}</textarea>
                            ...
                            <div class="flex items-center gap-[12px]">
                                ...
                                <label class="relative cursor-pointer">
                                    <input type="checkbox" name="activo" value="1" class="sr-only peer"
                                        {{ old('activo', $documentoLegal->activo) ? 'checked' : '' }}>
                ...
                <div class="trezo-card mb-[25px] bg-white dark:bg-[#0c1427] p-[20px] md:p-[25px] rounded-md">
                    <div class="flex justify-end gap-[15px]">
                        <a href="{{ route('dashboard.documentos-legales.show', $documentoLegal) }}"
