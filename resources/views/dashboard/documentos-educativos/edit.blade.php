...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <form method="POST" action="{{ route('dashboard.documentos-educativos.update', $documento) }}" enctype="multipart/form-data">
                @csrf @method('PUT')
                ...
                <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                        ...
                        <div>
                            ...
                            <select name="hogar_miembro_id" id="selMiembro" class="w-full">
                                ...
                                @foreach($miembros as $m)
                                    ...
                                    <option value="{{ $m->id }}" {{ old('hogar_miembro_id', $documento->hogar_miembro_id) == $m->id ? 'selected' : '' }}>{{ $lbl }}</option>
                                @endforeach
                            </select>
                            ...
                            <select name="tipo_documento_educativo_id" id="selTipo" class="w-full">
                                ...
                                @foreach($tiposDocumento as $td)
                                    <option value="{{ $td->id }}" data-icono="{{ $td->icono ?? 'description' }}"
                                        {{ old('tipo_documento_educativo_id', $documento->tipo_documento_educativo_id) == $td->id ? 'selected' : '' }}>
                                        {{ $td->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        {{-- Matrícula (subordinado al miembro) --}}
                        <div>
                            <label class="mb-[10px] text-black dark:text-white font-medium block">
                                Matrícula <span class="text-xs text-gray-400 font-normal ml-[4px]">(opcional)</span>
                            </label>
                ...
                <div class="trezo-card mb-[25px] bg-white dark:bg-[#0c1427] p-[20px] md:p-[25px] rounded-md">
                    <div class="flex justify-end gap-[15px]">
                        <a href="{{ route('dashboard.documentos-educativos.show', $documento) }}"
        ...
        <script>
            ...
            const initMatricula = '{{ old('matricula_id', $documento->matricula_id ?? '') }}';
