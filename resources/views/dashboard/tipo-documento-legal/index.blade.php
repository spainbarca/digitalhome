...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Tipos de Documento Legal</title>
    ...
    <body>
        ...
        @php
        $categoriaClasses = [
            'personal'   => 'text-blue-700 border-blue-600 bg-blue-100',
            'propiedad'  => 'text-green-700 border-green-600 bg-green-100',
            'seguro'     => 'text-orange-700 border-orange-600 bg-orange-100',
            'contrato'   => 'text-purple-700 border-purple-600 bg-purple-100',
            'denuncia'   => 'text-red-700 border-red-600 bg-red-100',
            'otro'       => 'text-gray-700 border-gray-600 bg-gray-100',
        ];
        $categoriaLabels = [
            'personal'   => 'Personal',
            'propiedad'  => 'Propiedad',
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Tipos de Documento Legal</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">Tipos de Documento Legal</li>
                </ol>
            </div>
                        ...
                        <table class="w-full">
                            ...
                            <tbody id="tablaBody">
                                ...
                                <tr class="border-b border-gray-100 dark:border-[#172036] hover:bg-gray-50 dark:hover:bg-[#15203c] transition-colors"
                                    ...
                                    <td class="py-[13px] px-[16px]">
                                        <div class="text-sm text-black dark:text-white font-medium">{{ $t->nombre }}</div>
                                        @if($t->descripcion)
                                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-[2px]">{{ Str::limit($t->descripcion, 60) }}</div>
                                        @endif
                                    </td>
                                    <td class="py-[13px] px-[16px]">
                                        @php $cls = $categoriaClasses[$t->categoria] ?? $categoriaClasses['otro']; @endphp
                                    ...
                                    <td class="py-[13px] px-[16px]">
                                        @if($t->requiere_vencimiento)
                                            <span class="inline-block text-xs font-medium px-[8px] py-[2px] rounded-[100px] text-warning-600 border border-warning-600 bg-warning-100 dark:bg-[#0c1427]">Sí</span>
                                        @else
        ...
        <div id="modalTipo" class="hidden fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="relative bg-white dark:bg-[#0c1427] rounded-md w-full max-w-md mx-[16px] shadow-2xl z-10 max-h-[90vh] overflow-y-auto">
                ...
                <div class="flex items-center justify-between px-[25px] py-[18px] border-b border-gray-100 dark:border-[#172036] sticky top-0 bg-white dark:bg-[#0c1427] z-10">
                    <h5 class="!mb-0 text-[16px]" id="modalTitulo">Nuevo Tipo de Documento Legal</h5>
                ...
                <div class="px-[25px] py-[20px] space-y-[16px]">
                    ...
                    <div>
                        ...
                        <input type="text" id="modalNombre" maxlength="255" placeholder="Ej: Escritura pública"
                    ...
                    <div>
                        ...
                        <textarea id="modalDescripcion" maxlength="500" rows="2" placeholder="Descripción opcional..."
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-[10px]">
                            <input type="text" id="modalIcono" maxlength="100" placeholder="Ej: description"
                            ...
                            <div class="flex-shrink-0 w-[46px] h-[46px] rounded-md border border-gray-200 dark:border-[#172036] flex items-center justify-center bg-primary-50 dark:bg-[#15203c]">
                                <i class="material-symbols-outlined !text-[22px] text-primary-500" id="previewIcono">description</i>
                            </div>
                        </div>
                    ...
                    <div>
                        <label class="mb-[8px] text-black dark:text-white font-medium block text-sm">Categoría <span class="text-danger-500">*</span></label>
                        ...
                        <p id="errCategoria" class="hidden text-xs text-danger-500 mt-[5px]"></p>
                    </div>

                    <div class="flex items-center justify-between pt-[4px]">
                        <span class="text-sm font-medium text-black dark:text-white">Requiere vencimiento</span>
                        <label class="relative cursor-pointer">
                            <input type="checkbox" id="modalRequiereVencimiento" class="sr-only peer">
        ...
        <script>
            ...
            const storeUrl   = '{{ route('dashboard.tipo-documento-legal.store') }}';
            const baseUrl    = '{{ url('dashboard/tipo-documento-legal') }}';
            const iconDefault = 'description';
            ...
            const categoriaLabels = {
                personal: 'Personal', propiedad: 'Propiedad', seguro: 'Seguro',
                contrato: 'Contrato', denuncia: 'Denuncia', otro: 'Otro',
            };
            ...
            function abrirModalCrear() {
                ...
                document.getElementById('modalTitulo').textContent              = 'Nuevo Tipo de Documento Legal';
            ...
            function abrirModalEditar(fila) {
                ...
                document.getElementById('modalTitulo').textContent              = 'Editar Tipo de Documento Legal';
            ...
            function badgeCategoria(cat) {
                const cls   = categoriaClasses[cat] ?? categoriaClasses['otro'];
            ...
            function badgeVencimiento(req) {
                return req
                    ? '<span class="inline-block text-xs font-medium px-[8px] py-[2px] rounded-[100px] text-warning-600 border border-warning-600 bg-warning-100">Sí</span>'
            ...
            function prependarFila(t) {
                ...
                tr.innerHTML = `
                    ...
                    <td class="py-[13px] px-[16px]">
                        <div class="text-sm text-black font-medium">${esc(t.nombre)}</div>
                        ${t.descripcion ? `<div class="text-xs text-gray-500 mt-[2px]">${esc(t.descripcion.substring(0, 60))}${t.descripcion.length > 60 ? '…' : ''}</div>` : ''}
                    </td>
            ...
            function mostrarErrores(errors) {
                const mapa = { nombre: 'errNombre', descripcion: 'errDescripcion', icono: 'errIcono', categoria: 'errCategoria' };
            ...
            function limpiarErrores() {
                ['errNombre', 'errDescripcion', 'errIcono', 'errCategoria'].forEach(id => {
                    const el = document.getElementById(id);
                    el.textContent = '';
