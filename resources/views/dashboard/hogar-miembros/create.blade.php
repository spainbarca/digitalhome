...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <form method="POST" action="{{ route('dashboard.hogares.miembros.store', $hogar) }}">
                @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
                            ...
                            <div>
                                ...
                                <select name="persona_id"
                                    class="h-[55px] rounded-md text-black dark:text-white border @error('persona_id') border-danger-500 @else border-gray-200 dark:border-[#172036] @enderror bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 focus:border-primary-500">
                                    <option value="">Seleccionar persona...</option>
                                    @foreach ($personas as $persona)
                                        <option value="{{ $persona->id }}" {{ old('persona_id', request('persona_id')) == $persona->id ? 'selected' : '' }}>
