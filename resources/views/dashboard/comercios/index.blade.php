        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                ...
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">Comercios</li>
                </ol>
            </div>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="flex flex-wrap items-center gap-[12px] justify-between">
                    <form method="GET" action="{{ route('dashboard.comercios.index') }}" class="flex flex-wrap items-center gap-[10px]">
                    ...
                    <a href="{{ route('dashboard.comercios.create') }}"
                ...
                <div class="trezo-card-content">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-[25px]">
                        ...
                        @php
                            ...
                            $bgColor   = $colores[abs(crc32($com->id)) % count($colores)];
                            $tipoIcono = $com->tipoComercio?->icono ?? 'store';
                            $nombre    = $com->nombre_referencial ?? $com->empresa?->razon_social ?? '(Sin nombre)';
                        @endphp
                        <div class="bg-white dark:bg-[#0c1427] p-[10px] rounded-md">
                            ...
                            <div class="relative h-[160px] rounded-md overflow-hidden">
                                @if($com->imagen_path)
                                    <img src="{{ asset('storage/' . $com->imagen_path) }}" class="w-full h-full object-cover object-top" alt="{{ $nombre }}">
                                @else
                                ...
                                @endif
                                @if($com->es_online)
                                <span class="absolute top-[8px] ltr:right-[8px] rtl:left-[8px] inline-flex items-center gap-[4px] text-[10px] font-semibold px-[7px] py-[2px] rounded-full bg-primary-500 text-white">
                            ...
                            <div class="text-center -mt-[26px]">
                                <div class="relative mb-[10px] inline-flex items-center justify-center w-[52px] h-[52px] rounded-full bg-white dark:bg-[#0c1427] ring-2 ring-white dark:ring-[#0c1427] shadow-sm overflow-hidden">
                                    @if($com->logo_efectivo)
                                        <img src="{{ $com->logo_efectivo }}" alt="{{ $com->empresa?->razon_social ?? $tipoIcono }}" class="w-full h-full object-contain">
                                    @elseif($com->empresa)
                                        <span class="text-lg font-bold text-gray-500 dark:text-gray-400 leading-none">{{ strtoupper(mb_substr($com->empresa->razon_social, 0, 1)) }}</span>
                                    @else
                                        <i class="material-symbols-outlined !text-[24px] text-gray-400">store</i>
                                    @endif
                                </div>
                                <h6 class="!text-sm !font-semibold !mb-[3px] truncate px-[8px]" title="{{ $nombre }}">{{ $nombre }}</h6>
                                @if($com->empresa)
                                    <span class="block text-xs text-gray-400 mb-[4px] truncate px-[8px]">{{ $com->empresa->razon_social }}</span>
                                @else
                                ...
                                <span class="block text-xs text-gray-500 dark:text-gray-400 mb-[12px]">
                                    @if($com->distrito_efectivo)
                                        <i class="material-symbols-outlined !text-[12px] align-middle">location_on</i>
                                        {{ $com->distrito_efectivo->distrito }}
                                    @else
                                        {{ $com->compras_count }} compra{{ $com->compras_count !== 1 ? 's' : '' }}
                                    @endif
                                </span>
                                ...
                                <div class="flex items-center justify-between gap-[6px] px-[4px]">
                                    <div class="flex items-center gap-[6px]">
                                        <i class="material-symbols-outlined !text-[18px] text-primary-400" title="{{ $com->tipoComercio?->nombre ?? 'Sin tipo' }}">{{ $tipoIcono }}</i>
                                        <span class="inline-block w-[9px] h-[9px] rounded-full {{ $com->activo ? 'bg-success-500' : 'bg-danger-400' }}" title="{{ $com->activo ? 'Activo' : 'Inactivo' }}"></span>
                                    </div>
                                    <a href="{{ route('dashboard.comercios.show', $com) }}"
                        ...
                        <div class="col-span-1 sm:col-span-2 md:col-span-3 xl:col-span-4 text-center py-[50px] text-gray-400">
                            <i class="material-symbols-outlined !text-[56px] mb-[10px] block text-gray-300 dark:text-gray-600">store</i>
                            @if(request('q') || request('tipo_id'))
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="sm:flex sm:items-center justify-between">
                    <p class="!mb-0 text-sm">
                        Mostrando {{ $comercios->firstItem() }}–{{ $comercios->lastItem() }} de {{ $comercios->total() }} resultados
                    </p>
                    <ol class="mt-[10px] sm:mt-0">
                        <li class="inline-block mx-[1px] ltr:first:ml-0 ltr:last:mr-0">
                            @if($comercios->onFirstPage())
                            ...
                            @else
                                <a href="{{ $comercios->previousPageUrl() }}" class="w-[31px] h-[31px] block leading-[29px] relative text-center rounded-md border border-gray-100 dark:border-[#172036] transition-all hover:bg-primary-500 hover:text-white hover:border-primary-500">
                                    <span class="opacity-0">0</span>
                                    <i class="material-symbols-outlined left-0 right-0 absolute top-1/2 -translate-y-1/2">chevron_left</i>
                                </a>
                            @endif
                        </li>
                        @foreach($comercios->getUrlRange(max(1,$comercios->currentPage()-2), min($comercios->lastPage(),$comercios->currentPage()+2)) as $page => $url)
                        <li class="inline-block mx-[1px]">
                            @if($page == $comercios->currentPage())
                        ...
                        <li class="inline-block mx-[1px] ltr:first:ml-0 ltr:last:mr-0">
                            @if($comercios->hasMorePages())
                                <a href="{{ $comercios->nextPageUrl() }}" class="w-[31px] h-[31px] block leading-[29px] relative text-center rounded-md border border-gray-100 dark:border-[#172036] transition-all hover:bg-primary-500 hover:text-white hover:border-primary-500">
                                    <span class="opacity-0">0</span>
