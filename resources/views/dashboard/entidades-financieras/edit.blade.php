...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Editar — {{ $entidadFinanciera->empresa?->razon_social ?? 'Entidad Financiera' }}</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Editar Entidad Financiera</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.entidades-financieras.show', $entidadFinanciera) }}" class="transition-all hover:text-primary-500 truncate">
                            {{ $entidadFinanciera->empresa?->razon_social ?? 'Detalle' }}
                        </a>
                    </li>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="trezo-card-header bg-gray-50 dark:bg-[#15203c] mb-[25px] flex items-center -mx-[20px] md:-mx-[25px] -mt-[20px] md:-mt-[25px] p-[20px] md:p-[25px] rounded-t-md">
                    <h5 class="!mb-0">Editar: {{ $entidadFinanciera->empresa?->razon_social ?? '—' }}</h5>
                </div>

                <div class="trezo-card-content">
                    <form method="POST" action="{{ route('dashboard.entidades-financieras.update', $entidadFinanciera) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-[25px]">
                            ...
                            <div class="grid grid-cols-12 gap-[20px] md:gap-[25px]">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="hidden" name="empresa_id" id="empresa_id" value="{{ old('empresa_id', $entidadFinanciera->empresa_id ?? '') }}">
                                ...
                                <input type="text" name="sigla" id="sigla"
                                    value="{{ old('sigla', $entidadFinanciera->sigla ?? '') }}"
                                    maxlength="50"
                                    placeholder="Ej. BCP"
                        ...
                        <div class="mb-[25px]">
                            <div class="flex items-center gap-[8px] mb-[15px] pb-[12px] border-b border-gray-100 dark:border-[#172036]">
                                ...
                                <h6 class="!mb-0 font-semibold text-black dark:text-white">2. Tipo de Entidad</h6>
                            </div>
                            <div id="tipoWrapper" class="relative">
                                <label class="mb-[10px] text-black dark:text-white font-medium block">Tipo de Entidad Financiera</label>
                                <div id="tipoTrigger"
                                    class="h-[55px] flex items-center rounded-md border {{ $errors->has('tipo_entidad_financiera_id') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[14px] cursor-pointer select-none transition-all hover:border-primary-500">
                                ...
                                <input type="hidden" name="tipo_entidad_financiera_id" id="tipo_entidad_financiera_id" value="{{ old('tipo_entidad_financiera_id', $entidadFinanciera->tipo_entidad_financiera_id ?? '') }}">
                                <div id="tipoDropdown"
                                    class="hidden absolute z-[50] w-full bg-white dark:bg-[#0c1427] border border-gray-200 dark:border-[#172036] rounded-md shadow-lg mt-[4px]">
                                    <div class="p-[8px] border-b border-gray-100 dark:border-[#172036]">
                                    ...
                                    <ul id="tipoOpciones" class="max-h-[220px] overflow-y-auto py-[4px]">
                                        <li class="tipo-opcion flex items-center gap-[8px] px-[12px] py-[9px] cursor-pointer hover:bg-primary-50 dark:hover:bg-[#15203c] transition-colors"
                                            data-id="" data-nombre="Sin tipo especificado" data-icono="account_balance">
                                        ...
                                        <li class="tipo-opcion flex items-center gap-[8px] px-[12px] py-[9px] cursor-pointer hover:bg-primary-50 dark:hover:bg-[#15203c] transition-colors"
                                            ...
                                            data-icono="{{ $t->icono ?? 'account_balance' }}">
                                            <i class="material-symbols-outlined !text-[18px] text-primary-500 flex-shrink-0">{{ $t->icono ?? 'account_balance' }}</i>
                            ...
                            @error('tipo_entidad_financiera_id')<p class="text-danger-500 text-xs mt-[5px]">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div class="mb-[25px]">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                                <div>
                                    <label class="mb-[10px] text-black dark:text-white font-medium block">Código SBS</label>
                                    <input type="text" name="codigo_sbs" value="{{ old('codigo_sbs', $entidadFinanciera->codigo_sbs) }}" maxlength="50"
                                        placeholder="Código SBS (Superintendencia)"
                                        class="h-[55px] rounded-md text-black dark:text-white border {{ $errors->has('codigo_sbs') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 transition-all placeholder:text-gray-500 focus:border-primary-500">
                                    @error('codigo_sbs')<p class="text-danger-500 text-xs mt-[5px]">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    <label class="mb-[10px] text-black dark:text-white font-medium block">SWIFT/BIC</label>
                                    <input type="text" name="swift" value="{{ old('swift', $entidadFinanciera->swift) }}" maxlength="20"
                                        placeholder="Ej. BCPLPEPL"
                                        class="h-[55px] rounded-md text-black dark:text-white border {{ $errors->has('swift') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 transition-all placeholder:text-gray-500 font-mono focus:border-primary-500">
                                    @error('swift')<p class="text-danger-500 text-xs mt-[5px]">{{ $message }}</p>@enderror
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-[25px] grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                            ...
                            <div>
                                ...
                                <label class="mb-[10px] text-black dark:text-white font-medium block">
                                    Logo de la entidad <span class="text-xs text-gray-400 font-normal">(PNG/JPG/WebP, máx. 2 MB)</span>
                                </label>
                                <p class="text-xs text-gray-400 mb-[8px]">Si se deja vacío, se usa el logo de la empresa.</p>
                                <div id="logoPreviewWrap" class="{{ $entidadFinanciera->logo_resuelto ? '' : 'hidden' }} mb-[10px]">
                                    <img id="logoPreview" src="{{ $entidadFinanciera->logo_resuelto ?? '' }}" alt="Logo"
                            ...
                            <div>
                                ...
                                <div id="bannerPreviewWrap" class="{{ $entidadFinanciera->banner_url ? '' : 'hidden' }} mb-[10px]">
                                    <img id="bannerPreview" src="{{ $entidadFinanciera->banner_url ?? '' }}" alt="Banner"
                        ...
                        <div class="mt-[20px] flex justify-end gap-[15px]">
                            <a href="{{ route('dashboard.entidades-financieras.show', $entidadFinanciera) }}"
        ...
        <script>
            ...
            let currentEmpresaLogo = @json($entidadFinanciera->empresa?->logo_url ? asset('storage/' . $entidadFinanciera->empresa->logo_url) : null);
            ...
            function setLogoPreviewFromEmpresa(empresaLogoUrl) {
                ...
                return; // entidad tiene logo propio — empresa no lo sobreescribe
                @endif
                const wrap = document.getElementById('logoPreviewWrap');
                const img  = document.getElementById('logoPreview');
                if (currentEmpresaLogo) {
            ...
            (function () {
                ...
                const hidden   = document.getElementById('tipo_entidad_financiera_id');
                ...
                function aplicar(id, nombre, icono) {
                    ...
                    if (id) {
                        label.textContent = nombre;
                        label.className = 'text-black dark:text-white text-sm flex-1 truncate';
                        iconContainer.innerHTML = `<i class="material-symbols-outlined !text-[20px] text-primary-500">${icono || 'account_balance'}</i>`;
                    } else {
