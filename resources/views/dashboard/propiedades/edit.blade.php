...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Editar: {{ $propiedad->alias }}</title>
    ...
    <body>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="trezo-card-header bg-gray-50 dark:bg-[#15203c] mb-[20px] md:mb-[25px] flex items-center justify-between -mx-[20px] md:-mx-[25px] -mt-[20px] md:-mt-[25px] p-[20px] md:p-[25px] rounded-t-md">
                    <div class="trezo-card-title">
                        <h5 class="!mb-0">Editar: {{ $propiedad->alias }}</h5>
                    </div>
                    <div>
                        <a href="{{ route('dashboard.propiedades.show', $propiedad) }}" class="text-sm text-primary-500 hover:underline">
                            <i class="ri-eye-line mr-[4px]"></i>Ver detalle
                        </a>
                    </div>
                </div>

                <div class="trezo-card-content">
                    <form method="POST" action="{{ route('dashboard.propiedades.update', $propiedad) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                            ...
                            <div>
                                ...
                                <input type="text" name="alias" value="{{ old('alias', $propiedad->alias) }}"
                            ...
                            <div>
                                ...
                                <select name="tipo_inmueble_id"
                                    ...
                                    @foreach($tiposInmueble as $tipo)
                                        <option value="{{ $tipo->id }}" {{ old('tipo_inmueble_id', $propiedad->tipo_inmueble_id) == $tipo->id ? 'selected' : '' }}>
                                            {{ $tipo->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" name="referencia" value="{{ old('referencia', $propiedad->referencia) }}"
                            ...
                            <div class="sm:col-span-2">
                                ...
                                @php
                                    $currentUbicacion = old('ubicacion_tipo',
                                        $propiedad->distrito_inei ? 'peru' : ($propiedad->pais_iso2 ? 'extranjero' : 'peru')
                                    );
                                @endphp
                                <div class="flex items-center gap-[25px]">
                                    <label class="flex items-center gap-[8px] cursor-pointer">
                                        <input type="radio" name="ubicacion_tipo" value="peru" id="loc_peru"
                                            {{ $currentUbicacion === 'peru' ? 'checked' : '' }}
                                    ...
                                    <label class="flex items-center gap-[8px] cursor-pointer">
                                        <input type="radio" name="ubicacion_tipo" value="extranjero" id="loc_ext"
                                            {{ $currentUbicacion === 'extranjero' ? 'checked' : '' }}
                                            class="cursor-pointer accent-primary-500 w-[16px] h-[16px]">
                                        <span class="text-black dark:text-white">
                                ...
                                <div>
                                    ...
                                    <select id="departamento_id"
                                        ...
                                        @foreach($departamentos as $dep)
                                            <option value="{{ $dep->inei }}" {{ $selectedDepartamentoInei == $dep->inei ? 'selected' : '' }}>
                                                {{ $dep->departamento }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="pais_iso2" id="pais_iso2"
                                        ...
                                        @foreach($paises as $pais)
                                            <option value="{{ $pais->iso2 }}" {{ old('pais_iso2', $propiedad->pais_iso2) == $pais->iso2 ? 'selected' : '' }}>
                                                {{ $pais->nombre }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <div class="mb-[12px] flex items-center gap-[15px]">
                                    <img src="{{ $propiedad->avatar_url }}" alt="{{ $propiedad->alias }}"
                                        class="h-[80px] w-[80px] object-cover rounded-md border border-gray-200 dark:border-[#172036]">
                                    <span class="text-sm text-gray-500 dark:text-gray-400">Imagen actual. Sube una nueva para reemplazarla.</span>
                                </div>
                                @endif
                            ...
                            <div class="sm:col-span-2">
                                <label class="flex items-center gap-[10px] cursor-pointer">
                                    ...
                                    <input type="checkbox" name="activo" value="1" id="activo"
                                        {{ old('activo', $propiedad->activo) ? 'checked' : '' }}
                                        class="cursor-pointer accent-primary-500 w-[16px] h-[16px]">
        ...
        <script>
            ...
            const initDepartamentoInei = "{{ $selectedDepartamentoInei }}";
            const initProvinciaInei    = "{{ $selectedProvinciaInei }}";
            const initDistritoInei     = "{{ old('distrito_inei', $propiedad->distrito_inei) }}";
            const initPaisIso2         = "{{ old('pais_iso2', $propiedad->pais_iso2) }}";
            const initCiudadId         = "{{ old('ciudad_id', $propiedad->ciudad_id) }}";
            ...
            // Función para cargar provincias dado un departamento
            function cargarProvincias(deptoInei, selectedProvincia) {
                ...
                allProvincias
                    ...
                    .forEach(p => {
                        ...
                        if (p.inei === selectedProvincia) opt.selected = true;
                        provSel.appendChild(opt);
                    });
            }

            // Función para cargar distritos dado una provincia
            function cargarDistritos(provInei, selectedDistrito) {
                ...
                allDistritos
                    ...
                    .forEach(d => {
                        ...
                        if (d.inei === selectedDistrito) opt.selected = true;
                        distSel.appendChild(opt);
                    });
            }

            // Función para cargar ciudades dado un país
            function cargarCiudades(paisIso2, selectedCiudad) {
                const ciudadSel = document.getElementById('ciudad_id');
                ciudadSel.innerHTML = '<option value="">Selecciona ciudad...</option>';
                allCiudades
                    .filter(c => c.pais_iso2 === paisIso2)
                    .forEach(c => {
                        const opt = document.createElement('option');
                        opt.value = c.id;
                        opt.textContent = c.nombre;
                        if (String(c.id) === selectedCiudad) opt.selected = true;
                        ciudadSel.appendChild(opt);
                    });
            }

            // Pre-poblar selectores al cargar la página
            if (initDepartamentoInei) {
                cargarProvincias(initDepartamentoInei, initProvinciaInei);
                if (initProvinciaInei) {
                    cargarDistritos(initProvinciaInei, initDistritoInei);
                }
            }

            if (initPaisIso2) {
                cargarCiudades(initPaisIso2, initCiudadId);
            }
            ...
            document.getElementById('pais_iso2').addEventListener('change', function () {
                cargarCiudades(this.value, null);
            });
