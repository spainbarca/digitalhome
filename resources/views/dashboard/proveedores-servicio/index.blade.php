...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Proveedores de Servicio</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Proveedores de Servicio</h5>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <div class="trezo-card-header mb-[20px] md:mb-[25px] flex flex-wrap items-center gap-[12px] justify-between">
                    <!-- Buscador + filtro -->
                    <form method="GET" action="{{ route('dashboard.proveedores.index') }}" class="flex flex-wrap items-center gap-[10px]">
                        ...
                        @if($search || $tipoId)
                        <a href="{{ route('dashboard.proveedores.index') }}"
                    ...
                    <a href="{{ route('dashboard.proveedores.create') }}"
                ...
                <div class="trezo-card-content">
                    <div class="table-responsive overflow-x-auto">
                        <table class="w-full">
                            <thead class="text-black dark:text-white">
                                <tr>
                                    <th class="font-medium ltr:text-left px-[20px] py-[11px] bg-gray-50 dark:bg-[#15203c] whitespace-nowrap ltr:first:rounded-tl-md ltr:last:rounded-tr-md">Logo</th>
                                    <th class="font-medium ltr:text-left px-[20px] py-[11px] bg-gray-50 dark:bg-[#15203c] whitespace-nowrap ltr:first:rounded-tl-md ltr:last:rounded-tr-md">Nombre</th>
                                    <th class="font-medium ltr:text-left px-[20px] py-[11px] bg-gray-50 dark:bg-[#15203c] whitespace-nowrap ltr:first:rounded-tl-md ltr:last:rounded-tr-md">Empresa</th>
                                    ...
                                    <th class="font-medium ltr:text-left px-[20px] py-[11px] bg-gray-50 dark:bg-[#15203c] whitespace-nowrap ltr:first:rounded-tl-md ltr:last:rounded-tr-md">Contacto</th>
                                    ...
                                    <th class="font-medium ltr:text-left px-[20px] py-[11px] bg-gray-50 dark:bg-[#15203c] whitespace-nowrap ltr:first:rounded-tl-md ltr:last:rounded-tr-md">Acciones</th>
                                </tr>
                            </thead>
                            <tbody class="text-black dark:text-white">
                                @forelse($proveedores as $p)
                                @php
                                    $nombreEfectivo = $p->nombre_comercial ?? $p->empresa?->razon_social ?? '(Sin nombre)';
                                    $colores = ['bg-primary-500','bg-orange-500','bg-success-500','bg-purple-500','bg-pink-500'];
                                    $bgColor = $colores[abs(crc32($p->id)) % count($colores)];
                                    $tipoIcono = $p->tipoServicio?->icono ?? 'home_repair_service';
                                @endphp
                                <tr>
                                    <td class="ltr:text-left whitespace-nowrap px-[20px] py-[15px] border-b border-gray-100 dark:border-[#172036] ltr:first:border-l ltr:last:border-r">
                                        @if($p->logo_url)
                                            <img src="{{ Storage::url($p->logo_url) }}"
                                    ...
                                    <td class="ltr:text-left whitespace-nowrap px-[20px] py-[15px] border-b border-gray-100 dark:border-[#172036] ltr:first:border-l ltr:last:border-r">
                                        <a href="{{ route('dashboard.proveedores.show', $p) }}"
                                            class="block font-medium text-[15px] hover:text-primary-500 transition-all">
                                            {{ $nombreEfectivo }}
                                        </a>
                                        @if($p->nombre_comercial && $p->empresa)
                                            <span class="block text-xs text-gray-500 dark:text-gray-400 mt-[2px]">{{ $p->empresa->ruc }}</span>
                                        @endif
                                    </td>
                                    <td class="ltr:text-left whitespace-nowrap px-[20px] py-[15px] border-b border-gray-100 dark:border-[#172036] ltr:first:border-l ltr:last:border-r text-sm">
                                        @if($p->empresa)
                                            <a href="{{ route('dashboard.empresas.show', $p->empresa) }}"
                                    ...
                                    <td class="ltr:text-left whitespace-nowrap px-[20px] py-[15px] border-b border-gray-100 dark:border-[#172036] ltr:first:border-l ltr:last:border-r text-sm text-gray-600 dark:text-gray-400">
                                        @if($p->telefono)
                                            <span class="flex items-center gap-[4px]">
                                                <i class="material-symbols-outlined !text-[14px]">phone</i>
                                                {{ $p->telefono }}
                                            </span>
                                        @endif
                                        @if($p->sitio_web)
                                            <a href="{{ $p->sitio_web }}" target="_blank" rel="noopener"
                                                class="flex items-center gap-[4px] text-primary-500 hover:underline truncate max-w-[160px] mt-[2px]">
                                                <i class="material-symbols-outlined !text-[14px]">language</i>
                                                {{ parse_url($p->sitio_web, PHP_URL_HOST) }}
                                            </a>
                                        @endif
                                        @if(!$p->telefono && !$p->sitio_web)
                                    ...
                                    <td class="ltr:text-left whitespace-nowrap px-[20px] py-[15px] border-b border-gray-100 dark:border-[#172036] ltr:first:border-l ltr:last:border-r">
                                        <div class="flex items-center gap-[9px]">
                                            ...
                                            <a href="{{ route('dashboard.proveedores.edit', $p) }}"
                                                class="text-gray-500 dark:text-gray-400 leading-none custom-tooltip" data-text="Editar">
                                                <i class="material-symbols-outlined !text-md">edit</i>
                                            </a>
                                            <form method="POST" action="{{ route('dashboard.proveedores.destroy', $p) }}" class="inline leading-none">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="text-danger-500 leading-none custom-tooltip" data-text="Eliminar"
                                                    onclick="return confirm('¿Eliminar el proveedor «{{ addslashes($nombreEfectivo) }}»?')">
                                ...
                                @empty
                                <tr>
                                    <td colspan="7" class="text-center px-[20px] py-[50px] text-gray-500 dark:text-gray-400">
                                        <i class="material-symbols-outlined !text-[48px] text-gray-300 dark:text-gray-600 block mb-[10px]">handshake</i>
                                        @if($search || $tipoId)
                    ...
                    <div class="px-[20px] py-[12px] md:py-[14px] rounded-b-md border-l border-r border-b border-gray-100 dark:border-[#172036] sm:flex sm:items-center justify-between">
                        <p class="!mb-0 text-sm">
                            Mostrando {{ $proveedores->firstItem() }}–{{ $proveedores->lastItem() }} de {{ $proveedores->total() }} resultados
                        </p>
                        <ol class="mt-[10px] sm:mt-0">
                            <li class="inline-block mx-[1px]">
                                ...
                                @else
                                    <a href="{{ $proveedores->previousPageUrl() }}" class="w-[31px] h-[31px] block leading-[29px] relative text-center rounded-md border border-gray-100 dark:border-[#172036] transition-all hover:bg-primary-500 hover:text-white hover:border-primary-500">
                                        <span class="opacity-0">0</span>
                                        <i class="material-symbols-outlined left-0 right-0 absolute top-1/2 -translate-y-1/2">chevron_left</i>
                                    </a>
                                @endif
                            ...
                            <li class="inline-block mx-[1px]">
                                @if($proveedores->hasMorePages())
                                    <a href="{{ $proveedores->nextPageUrl() }}" class="w-[31px] h-[31px] block leading-[29px] relative text-center rounded-md border border-gray-100 dark:border-[#172036] transition-all hover:bg-primary-500 hover:text-white hover:border-primary-500">
                                        <span class="opacity-0">0</span>
