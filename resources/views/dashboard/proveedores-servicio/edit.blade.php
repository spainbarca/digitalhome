...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        @php $nombreEfectivo = $proveedor->nombre_comercial ?? $proveedor->empresa?->razon_social ?? '(Sin nombre)'; @endphp
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                ...
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.proveedores.index') }}" class="transition-all hover:text-primary-500">Proveedores</a>
                    </li>
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">
                        <a href="{{ route('dashboard.proveedores.show', $proveedor) }}" class="transition-all hover:text-primary-500 truncate">{{ $nombreEfectivo }}</a>
                    </li>
            ...
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                ...
                <div class="trezo-card-content">
                    <form method="POST" action="{{ route('dashboard.proveedores.update', $proveedor) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-[25px]">
                            <div class="flex items-center gap-[8px] mb-[15px] pb-[12px] border-b border-gray-100 dark:border-[#172036]">
                                <i class="material-symbols-outlined !text-[20px] text-primary-500">electrical_services</i>
                                <h6 class="!mb-0 font-semibold text-black dark:text-white">1. Tipo de Servicio</h6>
                            </div>

                            <div class="max-w-lg">
                                <label class="mb-[10px] text-black dark:text-white font-medium block">
                                    Tipo de Servicio <span class="text-danger-500">*</span>
                                </label>
                                <div class="relative" id="tipoServicioWrapper">
                                    <div id="tipoServicioTrigger"
                                        class="h-[55px] flex items-center rounded-md border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[14px] cursor-pointer select-none transition-all hover:border-primary-500 {{ $errors->has('tipo_servicio_id') ? 'border-danger-500' : '' }}">
                                        <i class="material-symbols-outlined !text-[20px] mr-[8px] text-gray-400" id="tipoServicioIconPreview">home_repair_service</i>
                                        <span id="tipoServicioLabel" class="text-gray-500 dark:text-gray-400 text-sm flex-1 truncate">Seleccionar tipo de servicio...</span>
                                    ...
                                    <input type="hidden" name="tipo_servicio_id" id="tipo_servicio_id"
                                        value="{{ old('tipo_servicio_id', $proveedor->tipo_servicio_id) }}">
                                    <div id="tipoServicioDropdown"
                                        ...
                                        <ul id="tipoServicioOpciones" class="max-h-[240px] overflow-y-auto py-[4px]">
                                            ...
                                            <li class="tipo-opcion flex items-center gap-[8px] px-[12px] py-[9px] cursor-pointer hover:bg-primary-50 dark:hover:bg-[#15203c] transition-colors"
                                                ...
                                                data-icono="{{ $tipo->icono ?? 'home_repair_service' }}">
                                                <i class="material-symbols-outlined !text-[18px] text-primary-500">{{ $tipo->icono ?? 'home_repair_service' }}</i>
                                ...
                                @error('tipo_servicio_id')<p class="text-danger-500 text-xs mt-[5px]">{{ $message }}</p>@enderror
                            </div>
                        </div>
                        ...
                        <div class="mb-[25px]">
                            <div class="flex items-center gap-[8px] mb-[15px] pb-[12px] border-b border-gray-100 dark:border-[#172036]">
                                ...
                                <h6 class="!mb-0 font-semibold text-black dark:text-white">3. Datos del Proveedor</h6>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                                ...
                                <div>
                                    <label class="mb-[10px] text-black dark:text-white font-medium block">
                                        Nombre Comercial
                                        <span class="text-xs text-gray-400 font-normal ml-[4px]">(si difiere de la razón social)</span>
                                    </label>
                                    <input type="text" name="nombre_comercial" value="{{ old('nombre_comercial', $proveedor->nombre_comercial) }}" maxlength="150"
                                        class="h-[55px] rounded-md text-black dark:text-white border {{ $errors->has('nombre_comercial') ? 'border-danger-500' : 'border-gray-200 dark:border-[#172036]' }} bg-white dark:bg-[#0c1427] px-[17px] block w-full outline-0 transition-all placeholder:text-gray-500 focus:border-primary-500">
                                    @error('nombre_comercial')<p class="text-danger-500 text-xs mt-[5px]">{{ $message }}</p>@enderror
                                </div>

                                <div>
                                    <label class="mb-[10px] text-black dark:text-white font-medium block">Teléfono de Atención</label>
                                    <input type="text" name="telefono" value="{{ old('telefono', $proveedor->telefono) }}" maxlength="30"
                                ...
                                <div>
                                    ...
                                    <input type="url" name="sitio_web" value="{{ old('sitio_web', $proveedor->sitio_web) }}" maxlength="200"
                        ...
                        {{-- ── SECCIÓN 4: Logo y estado ────────────────────────────── --}}
                        <div class="mb-[25px]">
                            <div class="flex items-center gap-[8px] mb-[15px] pb-[12px] border-b border-gray-100 dark:border-[#172036]">
                                <i class="material-symbols-outlined !text-[20px] text-primary-500">image</i>
                                <h6 class="!mb-0 font-semibold text-black dark:text-white">4. Logo y Estado</h6>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-[20px] md:gap-[25px]">
                                ...
                                <div>
                                    ...
                                    <div class="mb-[10px] flex items-center gap-[10px]">
                                        <img src="{{ Storage::url($proveedor->logo_url) }}" id="logoPreview"
                                ...
                                <div class="flex items-center gap-[12px] pt-[10px] sm:pt-[30px]">
                                    ...
                                    <label class="relative cursor-pointer">
                                        <input type="checkbox" name="activo" value="1" class="sr-only peer" {{ old('activo', $proveedor->activo) ? 'checked' : '' }}>
                        ...
                        <div class="mt-[20px] flex justify-end gap-[15px]">
                            <a href="{{ route('dashboard.proveedores.show', $proveedor) }}"
        ...
        <script>
            ...
            // ── Custom select Tipo de Servicio ───────────────────────
            (function () {
                ...
                function aplicarSeleccion(id, nombre, icono) {
                    ...
                    if (id) {
                        ...
                        label.textContent  = 'Seleccionar tipo de servicio...';
                        label.className    = 'text-gray-500 dark:text-gray-400 text-sm flex-1 truncate';
                        iconPrev.textContent = 'home_repair_service';
