    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            @php
                ...
                $bgColor = $colores[abs(crc32($proveedor->id)) % count($colores)];
                $tipoIcono = $proveedor->tipoServicio?->icono ?? 'home_repair_service';
            @endphp
                    ...
                    <div class="trezo-card bg-white dark:bg-[#0c1427] p-[25px] rounded-md text-center">
                        <div class="flex justify-center mb-[16px]">
                            @if($proveedor->logo_url)
                                <img src="{{ Storage::url($proveedor->logo_url) }}"
                        ...
                        @if($proveedor->sigla_resuelta)
                            <span class="inline-block text-xs font-mono font-semibold px-[8px] py-[2px] rounded bg-gray-100 dark:bg-[#15203c] text-gray-500 dark:text-gray-400 mb-[8px]">{{ $proveedor->sigla_resuelta }}</span>
                        @endif

                        @if($proveedor->empresa)
                            <a href="{{ route('dashboard.empresas.show', $proveedor->empresa) }}"
                                class="block text-sm text-primary-500 hover:underline mb-[8px]">
                                {{ $proveedor->empresa->razon_social }}
                            </a>
                            <p class="text-xs text-gray-400 font-mono mb-[12px]">RUC: {{ $proveedor->empresa->ruc }}</p>
                        @endif
                        ...
                        <div class="flex flex-wrap justify-center gap-[6px] mb-[16px]">
                            ...
                            @endif
                            <span class="inline-block text-xs font-medium px-[8px] py-[3px] rounded-[100px] {{ $proveedor->activo ? 'text-success-600 border border-success-600 bg-success-100' : 'text-danger-600 border border-danger-600 bg-danger-100' }}">
                                {{ $proveedor->activo ? 'Activo' : 'Inactivo' }}
                            </span>
                        </div>

                        <div class="mt-[20px] pt-[16px] border-t border-gray-100 dark:border-[#172036] flex gap-[10px] justify-center">
                            <a href="{{ route('dashboard.proveedores.edit', $proveedor) }}"
                ...
                <div class="lg:col-span-2 space-y-[25px]">
                    ...
                    <div class="trezo-card bg-white dark:bg-[#0c1427] p-[25px] rounded-md">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[12px]">
                            <div>
                                <span class="block text-xs text-gray-500 dark:text-gray-400 mb-[2px]">Nombre comercial</span>
                                <span class="block text-sm font-medium text-black dark:text-white">{{ $proveedor->nombre_comercial ?: '—' }}</span>
                            </div>
                            <div>
                                <span class="block text-xs text-gray-500 dark:text-gray-400 mb-[2px]">Tipo de servicio</span>
                                <span class="block text-sm font-medium text-black dark:text-white flex items-center gap-[6px]">
                                    @if($proveedor->tipoServicio)
                                        ...
                                        {{ $proveedor->tipoServicio->nombre }}
                                        @if($proveedor->tipoServicio->unidadMedida)
                                            <span class="text-xs text-gray-400 font-mono">({{ $proveedor->tipoServicio->unidadMedida->simbolo }})</span>
                                        @endif
                                    @else
                                        —
                                    @endif
                    ...
                    <div class="trezo-card bg-white dark:bg-[#0c1427] p-[25px] rounded-md">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[12px]">
                            <div>
                                ...
                                <span class="block text-sm font-medium text-black dark:text-white">{{ $proveedor->telefono ?: '—' }}</span>
                            </div>
                            <div>
                                <span class="block text-xs text-gray-500 dark:text-gray-400 mb-[2px]">Sitio web</span>
                                @if($proveedor->sitio_web)
                                    <a href="{{ $proveedor->sitio_web }}" target="_blank" rel="noopener"
                                        class="block text-sm font-medium text-primary-500 hover:underline truncate">{{ $proveedor->sitio_web }}</a>
                                @else
                    ...
                    <div class="trezo-card bg-white dark:bg-[#0c1427] p-[25px] rounded-md">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-[12px]">
                            @php
                            $datosEmpresa = [
                                ['Razón social',  $proveedor->empresa->razon_social],
                                ['RUC',           $proveedor->empresa->ruc],
                                ['Estado SUNAT',  $proveedor->empresa->estado_sunat],
                                ['Condición',     $proveedor->empresa->condicion_sunat],
                                ['Sector',        $proveedor->empresa->sector?->nombre],
                            ];
                            @endphp
            ...
            <!-- Cuentas de servicio -->
            @if($proveedor->cuentasServicio->isNotEmpty())
            <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
                <h6 class="font-semibold text-black dark:text-white mb-[16px] flex items-center gap-[8px]">
                    <i class="material-symbols-outlined !text-[18px] text-primary-500">receipt_long</i>
                    Cuentas de Servicio vinculadas
                    <span class="ml-[6px] text-xs font-normal text-gray-400">({{ $proveedor->cuentasServicio->count() }})</span>
                </h6>
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead>
                            <tr>
                                <th class="font-medium ltr:text-left px-[20px] py-[11px] bg-gray-50 dark:bg-[#15203c] whitespace-nowrap ltr:first:rounded-tl-md ltr:last:rounded-tr-md">N° Cuenta</th>
                                <th class="font-medium ltr:text-left px-[20px] py-[11px] bg-gray-50 dark:bg-[#15203c] whitespace-nowrap">Titular</th>
                                <th class="font-medium ltr:text-left px-[20px] py-[11px] bg-gray-50 dark:bg-[#15203c] whitespace-nowrap">Estado</th>
                            </tr>
                        </thead>
                        <tbody class="text-black dark:text-white">
                            @foreach($proveedor->cuentasServicio as $cuenta)
                            <tr>
                                <td class="ltr:text-left whitespace-nowrap px-[20px] py-[13px] border-b border-gray-100 dark:border-[#172036] ltr:first:border-l ltr:last:border-r text-sm font-mono font-medium">
                                ...
                                <td class="ltr:text-left whitespace-nowrap px-[20px] py-[13px] border-b border-gray-100 dark:border-[#172036] ltr:first:border-l ltr:last:border-r text-sm">
                                    ...
                                    @else
                                        <span class="text-gray-400">Sin titular</span>
                                    @endif
                                </td>
                                <td class="ltr:text-left whitespace-nowrap px-[20px] py-[13px] border-b border-gray-100 dark:border-[#172036] ltr:first:border-l ltr:last:border-r">
                                    @php
                                        $badgeCuenta = match($cuenta->estado) {
                                            'activa'     => 'bg-success-50 text-success-600',
                                            'suspendida' => 'bg-warning-50 text-warning-600',
                                            default      => 'bg-danger-50 text-danger-500',
                                        };
                                    @endphp
                                    <span class="px-[8px] py-[3px] inline-block {{ $badgeCuenta }} dark:bg-[#15203c] rounded-sm font-medium text-xs capitalize">
                                        {{ $cuenta->estado }}
                                    </span>
                                </td>
                            </tr>
