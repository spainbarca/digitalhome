...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Modalidades Laborales</title>
    ...
    <body>
        ...
        <div class="main-content transition-all flex flex-col overflow-hidden min-h-screen" id="main-content">
            ...
            <div class="mb-[25px] md:flex items-center justify-between">
                <h5 class="!mb-0">Modalidades Laborales</h5>
                <ol class="breadcrumb mt-[12px] md:mt-0">
                    ...
                    <li class="breadcrumb-item inline-block relative text-sm mx-[11px] ltr:first:ml-0 rtl:first:mr-0 ltr:last:mr-0 rtl:last:ml-0">Modalidades</li>
                </ol>
            </div>
                            ...
                            <tbody id="tablaBody">
                                ...
                                <tr class="border-b border-gray-100 dark:border-[#172036] hover:bg-gray-50 dark:hover:bg-[#15203c] transition-colors"
                                    ...
                                    <td class="py-[13px] px-[16px]">
                                        <i class="material-symbols-outlined !text-[22px] text-primary-500">{{ $m->icono ?? 'work' }}</i>
                                    </td>
                                    <td class="py-[13px] px-[16px] text-sm text-black dark:text-white font-medium">{{ $m->nombre }}</td>
                                    <td class="py-[13px] px-[16px] text-sm text-gray-600 dark:text-gray-400">{{ $m->descripcion ?? '—' }}</td>
                                    <td class="py-[13px] px-[16px]">
                                        @if($m->activo)
                                ...
                                <tr id="filaVacia">
                                    <td colspan="5" class="text-center py-[50px] text-gray-500 dark:text-gray-400">
                                        <i class="material-symbols-outlined !text-[48px] text-gray-300 dark:text-gray-600 block mb-[10px]">work</i>
                                        No hay modalidades laborales registradas.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
        ...
        <!-- Modal -->
        <div id="modalModalidad" class="hidden fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true">
            ...
            <div class="relative bg-white dark:bg-[#0c1427] rounded-md w-full max-w-md mx-[16px] shadow-2xl z-10 max-h-[90vh] overflow-y-auto">
                ...
                <div class="flex items-center justify-between px-[25px] py-[18px] border-b border-gray-100 dark:border-[#172036] sticky top-0 bg-white dark:bg-[#0c1427] z-10">
                    <h5 class="!mb-0 text-[16px]" id="modalTitulo">Nueva Modalidad Laboral</h5>
                ...
                <div class="px-[25px] py-[20px] space-y-[16px]">
                    ...
                    <div>
                        ...
                        <input type="text" id="modalNombre" maxlength="255" placeholder="Ej: Tiempo completo"
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-[10px]">
                            <input type="text" id="modalIcono" maxlength="100" placeholder="Ej: work"
                            ...
                            <div class="flex-shrink-0 w-[46px] h-[46px] rounded-md border border-gray-200 dark:border-[#172036] flex items-center justify-center bg-primary-50 dark:bg-[#15203c]">
                                <i class="material-symbols-outlined !text-[22px] text-primary-500" id="previewIcono">work</i>
                            </div>
                        </div>
        ...
        <script>
            ...
            const storeUrl   = '{{ route('dashboard.modalidad-laboral.store') }}';
            const baseUrl    = '{{ url('dashboard/modalidad-laboral') }}';
            const iconDefault = 'work';
            ...
            function abrirModalCrear() {
                ...
                document.getElementById('modalTitulo').textContent  = 'Nueva Modalidad Laboral';
                ...
                limpiarErrores();
                document.getElementById('modalModalidad').classList.remove('hidden');
                document.getElementById('modalNombre').focus();
            }

            function abrirModalEditar(fila) {
                modoEdicion = true; idEditando = fila.dataset.id;
                document.getElementById('modalTitulo').textContent  = 'Editar Modalidad Laboral';
            ...
            function cerrarModal() {
                document.getElementById('modalModalidad').classList.add('hidden');
            }
            ...
            async function eliminar(id, nombre, fila) {
                if (!confirm(`¿Eliminar la modalidad «${nombre}»?`)) return;
                try {
            ...
            function prependarFila(m) {
                ...
                tr.dataset.id          = m.id;
                tr.dataset.nombre      = m.nombre;
                tr.dataset.descripcion = m.descripcion ?? '';
                tr.dataset.icono       = m.icono ?? '';
                tr.dataset.activo      = m.activo ? '1' : '0';
                tr.innerHTML = `
                    <td class="py-[13px] px-[16px]"><i class="material-symbols-outlined !text-[22px] text-primary-500">${esc(m.icono || iconDefault)}</i></td>
                    <td class="py-[13px] px-[16px] text-sm text-black font-medium">${esc(m.nombre)}</td>
                    <td class="py-[13px] px-[16px] text-sm text-gray-600">${esc(m.descripcion) || '—'}</td>
                    <td class="py-[13px] px-[16px]">${badgeActivo(m.activo)}</td>
                    <td class="py-[13px] px-[16px] text-right">${botonesHtml(m)}</td>`;
                document.getElementById('tablaBody').prepend(tr);
            }

            function actualizarFila(m) {
                const tr = document.querySelector(`#tablaBody tr[data-id="${m.id}"]`);
                if (!tr) return;
                tr.dataset.nombre      = m.nombre;
                tr.dataset.descripcion = m.descripcion ?? '';
                tr.dataset.icono       = m.icono ?? '';
                ...
                tds[0].innerHTML   = `<i class="material-symbols-outlined !text-[22px] text-primary-500">${esc(m.icono || iconDefault)}</i>`;
                tds[1].textContent = m.nombre;
                tds[2].textContent = m.descripcion || '—';
                tds[3].innerHTML   = badgeActivo(m.activo);
                tds[4].innerHTML   = `<div class="flex justify-end">${botonesHtml(m)}</div>`;
            }
            ...
            function mostrarFilaVacia() {
                ...
                tr.innerHTML = `<td colspan="5" class="text-center py-[50px] text-gray-500">
                    <i class="material-symbols-outlined !text-[48px] text-gray-300 block mb-[10px]">work</i>
                    No hay modalidades laborales registradas.</td>`;
                document.getElementById('tablaBody').appendChild(tr);
            }

            function mostrarErrores(errors) {
                const mapa = { nombre: 'errNombre', descripcion: 'errDescripcion', icono: 'errIcono' };
            ...
            function limpiarErrores() {
                ['errNombre', 'errDescripcion', 'errIcono'].forEach(id => {
                    const el = document.getElementById(id);
                    el.textContent = '';
